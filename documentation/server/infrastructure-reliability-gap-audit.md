...
- **Some operational documentation** — **Exists**, e.g. disaster-recovery notes in [`aws-resources.md`](./aws-resources.md), rollback *ideas* in [`Server/docs/validation-strict-mode-rollout.md`](../../Server/docs/validation-strict-mode-rollout.md) (e.g. flip `OPENAPI_VALIDATION_MODE` and restart).
...
- **Monitoring and alerting (Sentry, Datadog/New Relic, Pingdom-style uptime)**:
  - **Sentry (errors)** — **Not built.** [`Client/packages/services/security/errorReporter/ErrorReporterClass.ts`](../../Client/packages/services/security/errorReporter/ErrorReporterClass.ts) calls `initializeSentry` a **“placeholder”**; it only logs. The `@sentry` SDK is not present in the client dependency set used for this audit. “External” reporting is `sendToExternalService` → `clientErrorsApi` (your API), not Sentry. **No Sentry in Server** (no `sentry` / `datadog` / `newrelic` in `Server/requirements.txt` for this purpose).
...
- **Staging that mirrors production** — **Only partially reflected.** Staging base URLs appear in OpenAPI (e.g. [`openapi/openapi.yaml`](../../openapi/openapi.yaml), [`openapi/_onepath.yaml`](../../openapi/_onepath.yaml)). Internal checklists still note items such as “strict mode in staging: READY, not deployed” ([`.cursor/openapi-adoption-checklist.md`](../../.cursor/openapi-adoption-checklist.md)). There is **no** second deploy workflow or stack definition in **this** repository that proves prod-like parity (same pipeline, data tier, feature flags, etc.).
- **Rollback plan for deployments** — **Procedural / incomplete from repo evidence.** [`Server/docs/validation-strict-mode-rollout.md`](../../Server/docs/validation-strict-mode-rollout.md) describes config rollback. [`.github/workflows/ci_web.yml`](../../.github/workflows/ci_web.yml) deploys with image tag **`latest`** to ECR and EC2; there is **no** automated “deploy previous N” or blue/green step in the workflow. [`.cursor/openapi-adoption-checklist.md`](../../.cursor/openapi-adoption-checklist.md) still has unchecked “Rollback procedure tested.” **Roll forward / manual** is implied; a **tested, repo-backed rollback path is not complete.**
