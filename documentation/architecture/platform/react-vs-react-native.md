...
For shared UI primitives with platform splits, see [shared-ui-package.md](shared-ui-package.md). Prefer `.web.tsx` / `.native.tsx` (or `index.web.tsx` / `index.native.tsx`) so Vite and Metro resolve the correct implementation while shared packages stay RN-safe.
...
- **documentation/reference/linting.md** — How to run lint and other checks.
