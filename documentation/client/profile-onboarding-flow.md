...
1. Add the step id to [`profileStepIds.ts`](../../Client/packages/features/profile/types/profileStepIds.ts) if new.
...
Public APIs [`getOnboardingSteps`](../../Client/packages/features/profile/utils/onboarding/steps.ts) and [`getPersonalizationSteps`](../../Client/packages/features/profile/utils/onboarding/steps.ts) delegate to `buildProfileFlow`.

## Brokerage workspace shell

The **brokerage workspace placeholder** (dashboard + messaging only) is separate from this registry. See [workspaces-placeholder-shells.md](./workspaces-placeholder-shells.md). Onboarding registry work does not change placeholder routing.
