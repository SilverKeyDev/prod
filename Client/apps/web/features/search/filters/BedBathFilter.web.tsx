...
export default function BedBathFilter({
  ...
  return (
    <div className={`${compact ? "space-y-2" : "space-y-4"} ${className}`}>
      <BodyText
        size="sm"
        className={`font-medium text-gray-700 ${compact ? "mb-1" : "mb-2"}`}
      >
        {t("search.filters_beds_baths_range")}
      </BodyText>
      <div
        className={`grid grid-cols-1 sm:grid-cols-2 ${compact ? "gap-2" : "gap-4"}`}
      >
        <div className="touch-friendly">
          <BodyText
            as="div"
            size="xs"
            className={`${compact ? "mb-0.5" : "mb-1"} text-gray-500`}
          >
            {t("search.beds")}
          </BodyText>
          <BudgetRangeSlider
