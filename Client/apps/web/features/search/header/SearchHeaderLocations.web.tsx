...
import { BodyText, Button, Popover } from "@/components/ui/index.web";
import LocationSection from "@/features/profile/sections/LocationSection.web";
...
export default function SearchHeaderLocations({
  ...
  const locationTriggerClass =
    "flex h-11 min-w-0 flex-1 cursor-pointer items-center gap-1.5 overflow-x-auto scrollbar-hide rounded-lg border-2 border-dashed border-gray-300 bg-white/70 px-3 text-left transition-colors hover:border-olive hover:bg-white/90 touch-friendly";
      ...
      trigger={({ onToggle }) =>
        hasLocations ? (
          ...
          <Button
            ...
            variant="outline"
            size="sm"
            className="inline-flex h-11 items-center gap-2 border-2 border-dashed border-gray-300 bg-white/70 px-3 text-left text-gray-600 transition-colors hover:border-olive hover:bg-olive/5 hover:text-olive touch-friendly"
