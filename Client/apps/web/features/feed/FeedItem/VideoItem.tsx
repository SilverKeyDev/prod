...
import { Image, Video } from "@/components/ui/index.web";
...
export function VideoItem({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      const handler = () => {
        ...
        if (!document.hidden && hasError && isActive && isVisible) {
          setHasError(false);
        }
      };
  ...
  useEffect(() => {
    ...
    window.addEventListener(SEARCH_VIEW_MODE_CHANGED_EVENT, handler);
    return () =>
      window.removeEventListener(SEARCH_VIEW_MODE_CHANGED_EVENT, handler);
  }, []);
  ...
  useEffect(() => {
    ...
    window.addEventListener("reels-space-press", handler);
    return () => window.removeEventListener("reels-space-press", handler);
      ...
      if (accumulated > 0 && impressionIdRef.current) {
        ...
        if (media && media.duration > 0) {
          const watchedPercent = Math.min(
            100,
            (accumulated / 1000 / media.duration) * 100,
          );
  ...
  if (hasError) {
    return (
      <div
        className="relative h-full w-full min-h-0 snap-start overflow-hidden"
