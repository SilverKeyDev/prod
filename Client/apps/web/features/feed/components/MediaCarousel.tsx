...
import type { MediaItem } from "packages/schemas/content/feed/feed";
...
import { Button, Image, Region, Video } from "@/components/ui/index.web";
...
export type MediaCarouselProps = {
  ...
  /** When true, viewport allows horizontal pan (carousel swipe); when false, vertical scroll goes to feed. */
  gestureLockedToHorizontal?: boolean;
  /** Debug: report when the active slide video is playing */
  onVideoPlayingChange?: (playing: boolean) => void;
  /** When true, slide dots are not rendered (parent provides them). */
  hideSlideIndicator?: boolean;
  /** Called on tap (minimal movement); used for play/pause and double-tap like. Not called after a drag. */
  onTap?: () => void;
  className?: string;
};

/**
 * Full-screen horizontal carousel for a single reel. One slide at a time;
 * supports video (with conductor) and images. Dots only when media.length > 1 and hideSlideIndicator is false.
 */
export const MediaCarousel = forwardRef<MediaCarouselRef, MediaCarouselProps>(
  function MediaCarousel(
    ...
    const dragThreshold = isLargeScreen
      ? DRAG_THRESHOLD_LARGE
      : DRAG_THRESHOLD_SMALL;
    ...
    const shouldPlay =
      isReelActive && isActiveSlideVideo && autoplayEnabled && !userPaused;

    const activeVideoUrl =
      isActiveSlideVideo && activeItem.type === "video"
        ? activeItem.src
        : undefined;
    const isOnDeck =
      selectedIndex + 1 < media.length &&
      media[selectedIndex + 1]?.type === "video";
    ...
    // Only release gesture lock on pointer up/leave/cancel. Do NOT set lock on pointer down:
    // the parent's useFeedAxisLock decides vertical vs horizontal on first move; setting lock
    // on down would block vertical reel-to-reel scroll before the user has moved.
    ...
    return (
      <div
        className={className}
        style={{ minHeight: spacing(0), flex: 1, position: "relative" }}
      >
        <Region
          ref={setViewportRef}
          role="button"
