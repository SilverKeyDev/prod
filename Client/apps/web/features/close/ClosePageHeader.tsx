  ...
  return (
    <div className="mx-auto w-full max-w-[90vw] xl:px-6 2xl:px-8">
      ...
      >
        ...
        {/* Progress Bar */}
        {!loading && (
          <div className="mt-1 lg:mt-2 px-3">
        ...
        {activeTab && onTabChange && (
          <div className="mt-2 lg:mt-3 px-3">
            <div className="flex justify-center overflow-x-auto scrollbar-hide">
              {tabs.map((tab, index) => (
                <div key={tab.id} className="flex items-center flex-1 min-w-0">
                  <button
                    onClick={() => onTabChange(tab.id)}
                    className={`w-full px-1 lg:px-responsive-xs py-0.5 lg:py-responsive-2xs text-[7px] lg:text-xs font-medium transition-all duration-200 mx-0.5 ${
                      activeTab === tab.id
                        ? "text-olive border-b-2 border-olive"
                        : "text-navy/70 hover:bg-olive/10 hover:text-olive hover:shadow-sm rounded-lg"
                    }`}
                  >
                    <div className="text-center text-[7px] lg:text-xs font-medium">
                      {tab.label}
                    </div>
                  </button>
                  {index < tabs.length - 1 && (
                    <div className="h-4 lg:h-6 w-px bg-gray-300 mx-1 lg:mx-2 flex-shrink-0" />
                  )}
                </div>
              ))}
            </div>
