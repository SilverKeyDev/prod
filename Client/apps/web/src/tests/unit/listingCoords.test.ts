...
describe("getListingCoords", () => {
  ...
  it("parses string latitude/longitude from API payloads", () => {
    expect(
      getListingCoords({ latitude: "33.749", longitude: "-84.388" }),
    ).toEqual({
      lat: 33.749,
      lng: -84.388,
    });
  ...
  it("prefers lat over latitude when both are valid", () => {
    expect(
      getListingCoords({ lat: 1, latitude: 2, lng: 3, longitude: 4 }),
    ).toEqual({
      lat: 1,
      lng: 3,
    });
...
describe("getListingCoordsUnavailableDiagnostics", () => {
  it("returns null when coords are available", () => {
    expect(
      getListingCoordsUnavailableDiagnostics({ lat: 1, lng: 2 }),
    ).toBeNull();
  });

  it("reports missing coords when only zero placeholders are set", () => {
    expect(
      getListingCoordsUnavailableDiagnostics({ lat: 0, lng: 0 }),
    ).toMatchObject({
  ...
  it("reports invalid lat only", () => {
    expect(
      getListingCoordsUnavailableDiagnostics({ latitude: "nope", longitude: 1 })
        ?.reason,
    ).toBe("missing_or_invalid_lat");
  });

  it("reports invalid lng only", () => {
    expect(
      getListingCoordsUnavailableDiagnostics({ lat: 1, lng: "x" })?.reason,
    ).toBe("missing_or_invalid_lng");
  });
});
