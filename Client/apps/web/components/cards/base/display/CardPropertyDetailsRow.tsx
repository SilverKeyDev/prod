...
import { formatLotSizeInAcres } from "packages/utils/domain/search/address";
...
function formatSqftDisplay(
  ...
  if (sqft === undefined || Number(sqft) <= 0) return t("cards.na_sqft");
  return `${Math.round(Number(sqft)).toLocaleString()} ${t("cards.sqft")}`;
}

function formatLotDisplay(
  lotSize: string | undefined,
  t: (key: string) => string,
): string {
  if (!lotSize || lotSize === "N/A" || lotSize.trim() === "")
    return t("cards.no_data");
  const formatted = formatLotSizeInAcres(lotSize);
  return formatted ?? t("cards.no_data");
}
