  ...
  useEffect(() => {
    ...
    const run = () => {
      ...
      if (items && items.length > 0) {
        ...
        const contentEls = containerRef.current.querySelectorAll<HTMLElement>(
          ".aligned-row-content",
        );
        ...
        const contentHeights = Array.from(contentEls).map(
          (e) => e.offsetHeight,
        );
        const maxTitle = titleHeights.length ? Math.max(...titleHeights) : 0;
        const maxContent = contentHeights.length
    ...
    if (window.ResizeObserver) {
      ro = new ResizeObserver(() => setTimeout(run, 50));
      ro.observe(el);
    }
    return () => {
