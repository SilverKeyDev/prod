...
export const PropertyHeader: React.FC<PropertyHeaderProps> = ({
  ...
  const handleGenerateFullReport = useCallback(() => {
    ...
    // Property Details is a root-level screen; Saved is a tab inside Main. Navigate to Main -> Saved.
    (navigation as { navigate: (name: string, params?: { screen: string }) => void }).navigate(
      "Main",
      { screen: "Saved" }
    );
  }, [navigation, onGenerateReport, propertyAddress]);

  const handleShare = useCallback(async () => {
    const addr = getDisplayAddress(propertyAddress) ?? "";
    const p = property as { price?: unknown };
    let priceStr = "";
    if (typeof p.price === "number") priceStr = `$${p.price.toLocaleString()}`;
    if (typeof p.price === "string" && p.price.trim()) {
      priceStr = p.price.startsWith("$") ? p.price : `$${p.price}`;
    }
    const message = [addr, priceStr ? `Price: ${priceStr}` : null].filter(Boolean).join("\n");
    try {
  ...
  return (
    <View style={styles.header}>
      <Box className="flex-1 flex-row items-center justify-between border-b border-gray-200 bg-white px-4 py-3">
        {/* Left: Back + Title */}
