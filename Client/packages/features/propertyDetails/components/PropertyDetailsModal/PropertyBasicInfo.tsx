...
function formatPropertyPrimaryAddress(property: unknown): string {
...
type PropertyBasicInfoWebDetailsCardProps = {
...
function PropertyBasicInfoWebDetailsCard({
  propertyYearBuilt,
  propertyLotSize,
  propertyHomeType,
...
}: PropertyBasicInfoWebDetailsCardProps) {
  return (
    <Card className="p-4">
...
export const PropertyBasicInfo: React.FC<PropertyComponentProps & { isLoading?: boolean }> = ({
  ...
  const addressLine = formatPropertyPrimaryAddress(property);
  const showPriceSkeleton = isLoading && !hasRenderableListingPrice(propertyPrice);
  const showAddressSkeleton = isLoading && addressLine === ADDRESS_UNAVAILABLE;
  ...
  return (
    <Box className="p-6">
      ...
      <PropertyBasicInfoWebDetailsCard
