...
import { Tab } from "packages/ui/components/adapters/headless";
...
import AgreementOverviewTab from "@/features/documents/components/AgreementOverviewTab";
import AgreementRevisionsTab from "@/features/documents/components/AgreementRevisionsTab";
import AgreementStatusBadge from "@/features/documents/components/AgreementStatusBadge";
import EmbeddedSigning from "@/features/documents/components/EmbeddedSigning";
import ParticipantsList from "@/features/documents/components/ParticipantsList";
import {
...
export default function AgreementDetailModal({
  ...
  const { agreement, isLoading, error, refetchAgreement } = useDocusignAgreement(
    agreementId ?? undefined
  );
  const { sendAgreement, voidAgreement, isSendingAgreement, isVoidingAgreement } =
    useDocusignActions();
  ...
  const userCanCreateRevision =
    agreement && user ? canUserCreateRevision(agreement, user.id, isAgent) : false;
        ...
        {error && (
          <div className="flex flex-1 items-center justify-center p-6">
            <div className="text-center">
              <BodyText size="md" className="mb-2 text-red-600">
