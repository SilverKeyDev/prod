 ...
 */
export function DocuSignLegalNotice({ variant, className = "mb-3" }: DocuSignLegalNoticeProps) {
  const { t } = useLocalization();
  const key = VARIANT_KEYS[variant];

  return (
    <Box className={`border-border bg-accent-muted rounded-lg border p-3 ${className}`.trim()}>
