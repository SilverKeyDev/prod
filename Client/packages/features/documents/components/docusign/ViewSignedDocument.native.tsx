...
export default function ViewSignedDocument({
  ...
  if (error) {
    return (
      <Box style={styles.centerContainer}>
        <AlertCircle size={48} color={color("rose.DEFAULT")} style={styles.errorIcon} />
        ...
        {onClose && (
          <Button variant="secondary" onPress={onClose} style={styles.button} iconName="x">
            Close
          </Button>
        )}
      </Box>
  ...
  if (!documentUrl) {
    return (
      <Box style={styles.centerContainer}>
        <AlertCircle size={48} color={color("neutral.400")} style={styles.errorIcon} />
