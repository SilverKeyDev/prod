...
import { calculateCalendarDateRange, navigateDate } from "@/features/calendar/utils/date";

import { CalendarConnectionPrompt } from "./view/CalendarConnectionPrompt";
import { CalendarHeader } from "./view/CalendarHeader";
import { CalendarView } from "./view/calendarView";
import { CreateEventModal } from "./view/CreateEventModal";
...
export function Calendar() {
  ...
  } = useClientEvents(selectedClientId, dateRange.timeMin, dateRange.timeMax, "primary", [
    "primary",
  ]);
    ...
    (calendarId: string, enabled: boolean) => {
      ...
      setEnabledCalendarIds((prev) => {
        ...
        saveTimeoutRef.current = setTimeout(async () => {
          try {
            await savePreferences(calendars || [], newSet, silverKeyCalendarIdRef.current);
  ...
  const handleVisibleDatesChange = useCallback((firstDate: Date, lastDate: Date) => {
  ...
  return (
    <div className="w-full">
      ...
      {(calendarsLoading || eventsLoading || (!isViewingOwnCalendar && clientEventsLoading)) && (
        <div className="mb-4 text-center text-sm text-gray-500">
