...
import { getDefaultWorkingHours } from "@/features/calendar/utils/scheduling";
...
export function useSilverKeyCalendar(buyerName?: string) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        const calendarId = await getOrCreateSilverKeyCalendar(buyerName);
        store.setSilverKeyCalendarId(calendarId);
        return calendarId;
      } catch (error) {
        const errorMessage =
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (calendarId) => {
      store.setSilverKeyCalendarId(calendarId);
      queryClient.setQueryData(queryKeys.scheduling.silverKeyCalendar(), calendarId);
    },
  });

  return {
    calendarId: query.data || store.silverKeyCalendarId,
