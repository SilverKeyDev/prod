...
const REQUIRED_PERMISSIONS = [
  "calendar_app_created", // SilverKey / app-created calendar events
  "calendar_freebusy", // Scheduling availability
] as const;
...
export function useGoogleCalendarPermissions(): UseGoogleCalendarPermissionsReturn {
  ...
  const { hasRequiredPermissions, isPartiallyEnabled, missingPermissions } = useMemo(() => {
    ...
    const hasAppCreated = Boolean(permissions.calendar_app_created?.granted);

    if (!hasAppCreated) {
      missing.push("calendar_app_created");
    } else {
