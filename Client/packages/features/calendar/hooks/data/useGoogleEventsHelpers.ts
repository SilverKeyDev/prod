...
export function resolveCalendarIds(
  ...
  if (params.calendarIds?.length) {
    const resolved = params.calendarIds.map(resolve).filter((id): id is string => id !== null);
 ...
 */
export function getFirstFetchError(results: UseQueryResult<GoogleEvent[], Error>[]): string | null {
  for (const result of results) {
    if (result.error) {
