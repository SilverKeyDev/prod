...
describe("getEventLocalDayKeys", () => {
  it("includes each calendar day for multi-day all-day events (exclusive end)", () => {
    ...
    expect(getEventLocalDayKeys(ev)).toEqual([
      "2026-06-10",
      "2026-06-11",
      "2026-06-12",
    ]);
...
describe("eventSpansMultipleLocalDays", () => {
  it("is true for multi-day all-day range", () => {
    expect(
      eventSpansMultipleLocalDays(allDayEvent("2026-01-01", "2026-01-04")),
    ).toBe(true);
  });

  it("is false for single-day all-day", () => {
    expect(
      eventSpansMultipleLocalDays(allDayEvent("2026-01-01", "2026-01-02")),
    ).toBe(false);
  });
});
...
describe("getEventFirstLocalDayKey", () => {
  it("returns first day of range", () => {
    expect(
      getEventFirstLocalDayKey(allDayEvent("2026-03-01", "2026-03-05")),
    ).toBe("2026-03-01");
  });
});
