...
const SCOPES: readonly DevUserDataResetScope[] = ["profile", "preferences", "docusign"] as const;
...
export function AdminDevDataResetSection() {
  ...
  const handleReset = useCallback(async () => {
    ...
    if (!acknowledged) {
      setErrorMessage(t("admin.dev_reset.error_ack_required"));
      return;
    }
    try {
  ...
  }, [acknowledged, isSuperAdmin, mutation, scopeLabel, selectedScopes, t, targetUserId]);
  ...
  const canSubmit = selectedScopes.length > 0 && acknowledged && !busy;
  ...
  return (
    <Box className="border-border mt-8 border-t pt-6">
      ...
      <Label size="sm" className="mb-4 flex items-start gap-2">
        <AccessibleCheckboxInput
          checked={acknowledged}
          className="border-border accent-primary focus:ring-primary/30 mt-0.5 h-4 w-4 shrink-0 rounded focus:outline-none focus:ring-2 focus:ring-offset-0"
          label={t("admin.dev_reset.ack_label")}
          onChange={() => setAcknowledged((v) => !v)}
          disabled={busy}
        />
        <BodyText as="span" size="sm">
          {t("admin.dev_reset.ack_label")}
        </BodyText>
      </Label>
