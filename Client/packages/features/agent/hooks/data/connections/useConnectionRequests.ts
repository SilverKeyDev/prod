...
import { initiatedConnectionRequestsQueryKey } from "./useInitiatedConnectionRequests";
...
export type UseConnectionRequestsOptions = {
  /**
   * When false, skips inbox polling (use for agents: client→agent requests are auto-accepted
   * server-side). Clients keep inbox enabled to accept or reject agent outreach.
   */
  inboxEnabled?: boolean;
};

/**
 * Hook to manage connection requests (create, respond, inbox list).
 * Inbox fetch is client-only; agents auto-accept inbound client requests on the server.
 */
export function useConnectionRequests(
  options: UseConnectionRequestsOptions = {}
): UseConnectionRequestsReturn {
  const { inboxEnabled = true } = options;
  ...
  const {
    ...
    enabled: authReady && isAuthenticated && inboxEnabled,
