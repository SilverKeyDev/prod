...
const GoogleSignInButton: React.FC<GoogleSignInButtonProps> = ({
  ...
  const handleGoogleSignIn = () => {
    // Redirect to backend Google OAuth endpoint (platform window for RN parity)
    const apiUrl = getEnv().isDevelopment ? "http://localhost:5000" : "https://usesilverkey.com";
    const win = getWindow();
    if (win?.location) win.location.href = `${apiUrl}/api/v1/auth/google/start`;
  };
  ...
  return (
    <Button
      ...
      fullWidth
      className={`border-gray-300 bg-white text-gray-700 hover:bg-gray-50 ${className}`}
    >
