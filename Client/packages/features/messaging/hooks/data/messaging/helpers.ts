 ...
 */
export function mapApiMessagesToChatMessages(apiMessages: ApiMessageForMapping[]): ChatMessage[] {
...
export function formatMessageTime(date: Date): string {
  const d = date instanceof Date && !isNaN(date.getTime()) ? dayjs(date) : dateNow();
  const now = dateNow();
