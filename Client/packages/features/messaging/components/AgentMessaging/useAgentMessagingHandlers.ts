...
// Parity note: this hook is used by both web (`AgentMessaging`) and mobile (`MessagingScreen.native`).
...
export function useAgentMessagingHandlers({
  ...
  const handleSelectHome = useCallback(
    async (home: SavedHome) => {
      ...
      try {
        await sendMessageWithAttachment(conversationId, "", selectedClientId, propertyId);
        setShowSelectHomeModal(false);
      } catch (error) {
        log.error(LOG_CATEGORIES.MESSAGES, "Error sharing home", error);
      }
    },
    [selectedClientId, activeConversationId, sendMessageWithAttachment, setShowSelectHomeModal]
  );
  ...
  const handleSelectDocument = useCallback(
    ...
    [selectedClientId, activeConversationId, sendMessageWithAttachment, setShowSelectDocumentModal]
  );
