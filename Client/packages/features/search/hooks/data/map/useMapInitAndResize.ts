...
export function useMapInitAndResize(params: {
  ...
  const getVisibleMapEl = useCallback(() => {
    ...
    return isDesktop ? params.desktopMapRef.current : params.mobileMapRef.current;
  ...
  useEffect(() => {
    ...
    setTimeout(() => {
      if (
        winForResize &&
        (winForResize as Window & { google?: typeof google }).google?.maps?.event &&
        googleMapRef.current
      ) {
        (winForResize as Window & { google: typeof google }).google.maps.event.trigger(
          googleMapRef.current,
          "resize"
        );
      }
  ...
  const ensureMapMounted = useCallback(() => {
    ...
    setTimeout(() => {
      if (
        winForResize2 &&
        (winForResize2 as Window & { google?: typeof google }).google?.maps?.event &&
        googleMapRef.current
      ) {
        (winForResize2 as Window & { google: typeof google }).google.maps.event.trigger(
          googleMapRef.current,
          "resize"
        );
      }
