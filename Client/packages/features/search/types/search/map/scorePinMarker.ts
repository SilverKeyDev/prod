...
export function createScorePinElement(score: number): HTMLElement {
  ...
  const { fillColor, strokeColor } = getMapPinColorsForScoreAndStatus(safeScore);
  const safeFill = sanitizeCssHexColor(fillColor, fillColor);
  const safeStroke = sanitizeCssHexColor(strokeColor, strokeColor);
  ...
  wrapper.innerHTML = `
    <svg width="${size}" height="${size}" viewBox="0 0 24 36" fill="none" xmlns="http://www.w3.org/2000/svg" style="display: block;">
      ...
      />
      <circle cx="12" cy="12" r="4" fill="${safeStroke}" opacity="0.6"/>
    </svg>
  `;
  return wrapper;
}
