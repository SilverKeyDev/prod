...
/** Correlates client ↔ Flask ↔ Slipstream logs via `X-Request-Id`. */
function createSearchCorrelationId(): string {
  try {
  ...
  return `sk_${Date.now().toString(36)}_${Math.random().toString(36).slice(2, 11)}`;
}
