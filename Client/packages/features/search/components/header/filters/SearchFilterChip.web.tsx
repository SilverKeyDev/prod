...
const chipBase = `inline-flex touch-friendly shrink-0 items-center gap-1.5 rounded-lg px-4 text-left text-sm font-medium transition-colors min-w-0 overflow-hidden ${HEADER_ROW_CONTROL_HEIGHT}`;
...
export type SearchFilterChipProps = {
  ...
  summary: string;
  /** Panel content; receives onClose and optional portal registration for nested portaled menus */
  children: (props: {
    onClose: () => void;
    panelId?: string;
...
export default function SearchFilterChip({
  ...
  return (
    ...
    >
      {(panelProps) => children(panelProps)}
    </Popover>
  );
}
