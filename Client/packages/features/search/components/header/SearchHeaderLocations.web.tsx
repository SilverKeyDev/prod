...
export default function SearchHeaderLocations({
  ...
  const locationTriggerClass =
    "box-border flex h-11 min-h-11 max-h-11 min-w-0 flex-1 cursor-pointer items-center justify-between gap-2 rounded-lg border-2 border-dashed border-gray-300 bg-white/70 px-3 text-left transition-colors hover:border-olive hover:bg-white/90 touch-friendly";
  const locationPanelClass =
  ...
  return (
    <Popover
      ...
      usePortal={true}
      side="left"
      panelClassName={locationPanelClass}
      panelMaxHeight="85vh"
      panelMinWidth="320px"
      ...
      trigger={({ onToggle }) =>
        hasLocations ? (
          ...
          >
            ...
            <Pencil className="h-3.5 w-3.5 shrink-0 text-gray-400" aria-hidden />
          </Button>
        ) : (
          <Button
            type="button"
            ...
            variant="outline"
            size="sm"
            className="hover:border-olive hover:bg-olive/5 hover:text-olive touch-friendly box-border inline-flex h-11 max-h-11 min-h-11 min-w-0 flex-1 items-center justify-start gap-2 border-2 border-dashed border-gray-300 bg-white/70 px-3 text-left text-gray-600 transition-colors"
