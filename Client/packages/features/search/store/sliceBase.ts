...
function baseSearchSetters(set: SetState): Partial<ConsolidatedSearchState> {
  return {
    setSearchResults: (results) =>
      set((state) =>
        arraysShallowEqual(state.searchResults, results) ? state : { searchResults: results }
      ),
    ...
    loadSearchResultsFromCache: (preferencesVersion = "1.0") => {
      try {
        ...
        if (cachedResults && cachedResults.length > 0) {
          const propertyDetails: PropertyDetails[] = cachedResults.map((result) => ({
            id: result.id,
            ...
            propertyType: (result.propertyType as PropertyType) || "SINGLE_FAMILY",
            listingStatus: (result.listingStatus as ListingStatus) || "FOR_SALE",
...
function baseUISetters(set: SetState, get: GetState): Partial<ConsolidatedSearchState> {
  return {
...
function baseStubSetters(get: GetState): Partial<ConsolidatedSearchState> {
  return {
    ...
    performSearch: () => Promise.resolve({ success: false, error: "Not implemented" }),
    loadMoreResults: () => Promise.resolve({ success: false, error: "Not implemented" }),
  };
}
