...
export type TaskChecklistResponse =
  components["schemas"]["TaskChecklistResponse"];
export type TaskChecklistApiResponse =
  components["schemas"]["TaskChecklistApiResponse"];
...
export type TransactionAddressData =
  components["schemas"]["TransactionAddressData"];
export type TransactionAddressResponse =
  components["schemas"]["TransactionAddressResponse"];

export async function getTransactionAddress(): Promise<TransactionAddressData | null> {
  const response = await apiGet<TransactionAddressResponse>(
...
export async function saveTransactionAddress(
  ...
  const response = await apiPost<TransactionAddressResponse>(
    "/api/v1/transactions/address",
    data,
  );
  if (!response.success || !response.data) {
