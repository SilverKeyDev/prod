...
export function handleReelsKeyDown(
  ...
  const container = doc?.querySelector("[data-reels-feed-container]") as HTMLElement | null;
...
export function handleReelsWheel(
  ...
  e.preventDefault();
  const direction = e.deltaY > 0 ? (1 as const) : e.deltaY < 0 ? (-1 as const) : null;
...
export function handleReelsMouseUp(
  ...
  const container = doc?.querySelector("[data-reels-feed-container]") as HTMLElement | null;
  if (!container) return;
  // Only run drag/scroll if the mouseup target is inside the reels container.
  // Otherwise a click on the sidebar (or any nav) would be treated as a drag end and could
