...
export function useFeedAxisLock(options?: UseFeedAxisLockOptions) {
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent, containerRef: React.RefObject<HTMLElement | null>) => {
      const t = e.touches[0];
      ...
      if (!decidedRef.current && dist >= deadzonePx) {
        ...
        const isVertical = angle <= ANGLE_THRESHOLD_DEG || angle >= 180 - ANGLE_THRESHOLD_DEG;
        lockRef.current = isVertical ? "vertical" : "horizontal";
        containerRef.current.style.touchAction = lockRef.current === "vertical" ? "pan-y" : "pan-x";
  ...
  const handleTouchEnd = useCallback((containerRef: React.RefObject<HTMLElement | null>) => {
