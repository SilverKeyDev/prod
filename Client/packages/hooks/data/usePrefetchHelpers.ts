...
export async function prefetchRoute(
  ...
  try {
    ...
    if (routeConfig.key === "googleEvents") {
      ...
      queryClient.setQueryData(calendarsQueryKey, typedResult.calendars);
      log.info(
        LOG_CATEGORIES.CALENDAR,
 ...
 */
export async function prefetchAllInitialData(
  params: PrefetchAllParams,
): Promise<void> {
