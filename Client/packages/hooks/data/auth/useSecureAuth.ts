...
function applyLoginSuccess(
  ...
  if (response.user) {
    const mappedUser = mapAuthResponseToUserProfile(
      response.user,
      response.user_sub,
    );
    ...
    if (getEnv().isDevelopment) {
      log.debug(LOG_CATEGORIES.AUTH, "User mapping", {
        ...
        responseUserSub:
          "user_sub" in response.user ? response.user.user_sub : undefined,
...
async function performLogin(
  ...
  try {
    ...
    if (response.needs_verification) {
      ...
      setError(
        response.message ?? "Please verify your email address to continue.",
      );
      setLoginRef(false);
...
async function performRefreshToken(setters: RefreshSetters): Promise<boolean> {
  ...
  try {
    const { authApi: api } = await import("../../../config/api");
