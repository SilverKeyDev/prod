...
import { schedulePreload } from "packages/utils/domain/feed";
...
export function applyRangeChanged(
  ...
  useFeedStore.getState().setUserPaused(false);
  schedulePreload(
  ...
  const shouldUpdateUrl =
    item && (feedIdFromUrl != null || idx !== initialIndex);
  if (shouldUpdateUrl) {
    setSearchParams(
      (prev) => {
 ...
 * Extracted to satisfy max-lines-per-function in useFeedScrollContainer.
 */
export function useFeedScrollViewport(
...
export function useFeedScrollControllerEffect(
  ...
  itemCount: number,
  scrollControllerRef:
    | React.MutableRefObject<FeedScrollController | null>
    | undefined,
) {
  useEffect(() => {
