 ...
 */
export function mapResultUserToProfile(
  resultUser: ResetResultUser,
  _email: string,
): UserProfile {
  const userId =
    resultUser.id ||
    ("user_sub" in resultUser ? resultUser.user_sub : undefined) ||
    undefined;
  return {
    id: userId || "",
    email: resultUser.email,
    name: resultUser.name || "Unknown User",
    phone: ("phone" in resultUser ? resultUser.phone : undefined) as
      | string
      | null
      | undefined,
    created_at: null,
    ...
    is_agent:
      ("is_agent" in resultUser ? (resultUser.is_agent ?? false) : false) ??
      false,
    auth_method: ("auth_method" in resultUser
      ? resultUser.auth_method
      : undefined) as "cognito" | "google" | "both" | "unknown" | undefined,
  };
}
