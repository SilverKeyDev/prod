...
export const CLIMATE_ENVIRONMENTAL_KEY = "climate_environmental_safety" as const;
...
export type EnvironmentalFactorScoreKey = (typeof ENVIRONMENTAL_FACTOR_SCORE_KEYS)[number];
...
export function hasEnvironmentalFactorsContent(raw: unknown): boolean {
  const unwrapped = unwrapPropertyAnalysisSection(CLIMATE_ENVIRONMENTAL_KEY, raw);
...
export function parseEnvironmentalSection(raw: unknown): ParsedEnvironmentalSection | null {
  const unwrapped = unwrapPropertyAnalysisSection(CLIMATE_ENVIRONMENTAL_KEY, raw);
  if (unwrapped == null || typeof unwrapped !== "object" || Array.isArray(unwrapped)) {
    return null;
  }
