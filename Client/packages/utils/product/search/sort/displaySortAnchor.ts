 ...
 */
export function resolveDistanceSortMode(input: DistanceAnchorInput): DistanceSortMode {
  ...
  const locs = input.importantLocations.filter(
    (p) => typeof p.lat === "number" && typeof p.lng === "number" && !Number.isNaN(p.lat + p.lng)
  );
...
export function distanceKmForSort(mode: DistanceSortMode, property: LatLng): number | null {
  if (mode.type === "none") return null;
  const plat = property.lat;
  const plng = property.lng;
  if (typeof plat !== "number" || typeof plng !== "number" || Number.isNaN(plat + plng)) {
    return null;
  }
