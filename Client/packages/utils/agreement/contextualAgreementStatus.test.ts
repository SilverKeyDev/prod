...
import {
  ...
  getNextSignerUserId,
} from "./contextualAgreementStatus";

const baseParticipants = (
  clientSigned: boolean,
  agentSigned: boolean,
): NonNullable<
  Parameters<typeof getContextualAgreementStatus>[0]["participants"]
> => [
  {
    user_id: "buyer-1",
    role: "signer",
...
describe("getContextualAgreementStatus", () => {
  it("only marks the current routing recipient as sign_now", () => {
    ...
    expect(getContextualAgreementStatus(agreement, "buyer-1", false)).toBe(
      "sign_now",
    );
    expect(getContextualAgreementStatus(agreement, "agent-1", true)).toBe(
