...
import { addressStreetLineForCard, formatFilenameToAddress } from "./addressFormatting";

describe("addressStreetLineForCard", () => {
  it("returns first segment for comma-separated US addresses", () => {
    expect(addressStreetLineForCard("123 Main St, Springfield, IL, 62701")).toBe("123 Main St");
    expect(addressStreetLineForCard("777 W Middlefield Rd, Mountain View, CA, 94043")).toBe(
  ...
  it("handles CA94043 in one trailing segment after city", () => {
    expect(addressStreetLineForCard("99 Pine Rd, Boulder, CO 80301")).toBe("99 Pine Rd");
  });
