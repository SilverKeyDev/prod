...
import { isChunkLoadError, STALE_DEPLOY_CHUNK_RELOAD_KEY } from "./chunkLoadRecovery";
...
vi.mock("packages/utils/platform", async (importOriginal) => {
  const actual = await importOriginal<typeof import("packages/utils/platform")>();
  return {
    ...actual,
    getWindow: () =>
      ({
...
describe("tryRecoverFromStaleDeployChunk", () => {
  ...
  it("does not reload in development", async () => {
    ...
    const { tryRecoverFromStaleDeployChunk } = await import("./chunkLoadRecovery");
