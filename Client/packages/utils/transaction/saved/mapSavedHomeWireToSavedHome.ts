...
import type { PropertySearchListingPriceSource } from "packages/utils/product/search/pricing/formatPropertySearchListingPrice";
...
/** Saved-home wire shape from API mutations (PropertyCache + UserPropertyLink fields). */
export interface SavedHomeWire {
  id?: string;
  home_id?: string;
  address?: string;
  price?: string;
...
/**
 * Map saved-home wire data to {@link SavedHome}. Expects API shape with optional id, address,
 * price, beds, baths, sqft, lot_size, image_url; lat/lng from lat|latitude and lng|longitude|lon.
 */
export function mapSavedHomeWireToSavedHome(home: unknown, index: number): SavedHome {
