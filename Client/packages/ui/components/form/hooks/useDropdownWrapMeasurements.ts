...
type UseDropdownWrapMeasurementsParams = {
  displayLabel: string;
  isOpen: boolean;
  /** Used for ResizeObserver deps when option list identity changes */
  filteredOptions: readonly unknown[];
};

/**
 * Measures trigger and option label wrapping (hidden measurer + per-option observers)
 * so long labels can adjust typography without layout thrash.
 */
export function useDropdownWrapMeasurements({
  displayLabel,
  isOpen,
  filteredOptions,
}: UseDropdownWrapMeasurementsParams) {
