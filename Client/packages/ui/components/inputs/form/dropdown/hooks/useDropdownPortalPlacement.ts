...
import { getScrollParents } from "packages/ui/components/inputs/form/dropdown/dropdownScrollParents";
  ...
  useLayoutEffect(() => {
    ...
    const updatePlacement = () => {
      ...
      if (menuPlacement === "overlap") {
        ...
        const maxHeight = Math.min(desiredMenuHeightPx, Math.max(48, maxByViewport));
        setPortalPlacement({
          top: centerY,
          left: rect.left,
        ...
        const availBelow = Math.max(0, win.innerHeight - rect.bottom - margin - gap);
