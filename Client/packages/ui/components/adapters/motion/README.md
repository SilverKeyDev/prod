...
Thin façade so feature code can import one path (`packages/ui/components/adapters/motion`). Metro / Vite resolve `.web` vs `.native` implementations.
...
- **Simple opacity fades (`MotionView`)** — Used in search / map UI (`Client/apps/web/pages/property/SearchPageContent.tsx`, `Client/packages/features/search/components/src/SearchFeature.tsx`, `Client/packages/features/search/components/layout/SearchPageMapView.tsx`). Native `MotionView` matches this subset.
