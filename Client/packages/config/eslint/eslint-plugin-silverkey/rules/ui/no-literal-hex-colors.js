 ...
 */
const TAILWIND_ARBITRARY_HEX_RE = /\[#([0-9A-Fa-f]{3}|[0-9A-Fa-f]{6}|[0-9A-Fa-f]{8})\]/g;
...
module.exports = {
    ...
    schema: [
      {
        ...
        properties: {
          includePaths: {
            type: "array",
            items: { type: "string" },
            description: "Glob-like paths where the rule applies (e.g. apps/web/components/**)",
          },
          allowedPaths: {
            type: "array",
            items: { type: "string" },
            description: "Paths where hex literals are allowed (e.g. packages/design-tokens/**)",
          },
        },
  ...
  create(context) {
    ...
    function hasArbitraryColorInString(str) {
      ...
      return TAILWIND_ARBITRARY_HEX_RE.test(str) || TAILWIND_ARBITRARY_FUNC_RE.test(str);
    }
