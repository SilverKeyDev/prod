...
/**
 * Flag apiGet<any>, apiPost<any>, etc. in packages/api (and packages/config/api barrel).
...
module.exports = {
  ...
  create(context) {
    ...
    if (!normalized.includes("packages/api") && !normalized.includes("packages/config/api"))
      return {};
    ...
    return {
      CallExpression(node) {
        ...
        const typeArgs =
          node.typeParameters || (node.typeArguments && { params: node.typeArguments.params });
