...
const baseCreator: import("zustand").StateCreator<SavedHomesState> = (set) => ({
  ...
  setSavedHomes: (homes) =>
    set((state) =>
      arraysShallowEqual(state.savedHomes, homes)
        ? state
        : { savedHomes: homes },
    ),
  setSavedHomesLoading: (loading) =>
    set((state) =>
      state.savedHomesLoading === loading
        ? state
        : { savedHomesLoading: loading },
    ),
  setSavedHomesError: (error) =>
    set((state) =>
      state.savedHomesError === error ? state : { savedHomesError: error },
    ),
  ...
  saveHome: () => {
    log.warn(
      LOG_CATEGORIES.HOOKS,
      "saveHome should be implemented by useSavedHomesData hook",
    );
...
const withReset = withResettable<SavedHomesState>(baseCreator, (set) => ({
  ...
  addSavedHome: (home) =>
    set((state) => ({ savedHomes: [...state.savedHomes, home] })),
  removeSavedHome: (homeId) =>
    set((state) => ({
      savedHomes: state.savedHomes.filter((home) => home.home_id !== homeId),
    })),
