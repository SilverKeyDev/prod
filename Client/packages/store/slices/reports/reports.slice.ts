...
const baseCreator: import("zustand").StateCreator<ReportsState> = (set) => ({
  ...
  setReports: (reports) =>
    set((state) =>
      arraysShallowEqual(state.reports, reports) ? state : { reports },
    ),
  setReportsLoading: (loading) =>
    set((state) =>
      state.reportsLoading === loading ? state : { reportsLoading: loading },
    ),
  setReportsError: (error) =>
    set((state) =>
      state.reportsError === error ? state : { reportsError: error },
    ),

  setCompareReports: (reports) =>
    set((state) =>
      arraysShallowEqual(state.compareReports, reports)
        ? state
        : { compareReports: reports },
    ),
  setCompareReportsLoading: (loading) =>
    set((state) =>
      state.compareReportsLoading === loading
        ? state
        : { compareReportsLoading: loading },
    ),
  setCompareReportsError: (error) =>
    set((state) =>
      state.compareReportsError === error
        ? state
        : { compareReportsError: error },
    ),
  ...
  refreshReports: async () => {
    log.warn(
      ...
      "refreshReports should be implemented by useReportsData hook",
    );
  ...
  refreshCompareReports: async () => {
    log.warn(
      LOG_CATEGORIES.HOOKS,
      "refreshCompareReports should be implemented by useReportsData hook",
    );
    return Promise.resolve();
  },
...
const withDev = withDevtools<ReportsState>("reports")(
  withPersist,
) as unknown as import("zustand").StateCreator<ReportsState>;
