...
export const useBillingStore = create<BillingState>()((set) => ({
  ...
  setBillingInfo: (info: BillingInfo | null) =>
    set((state) =>
      state.billingInfo === info ? state : { billingInfo: info },
    ),
  setBillingLoading: (loading: boolean) =>
    set((state) =>
      state.billingLoading === loading ? state : { billingLoading: loading },
    ),
  setBillingError: (error: string | null) =>
    set((state) =>
      state.billingError === error ? state : { billingError: error },
    ),
  setPaymentLoading: (loading: boolean) =>
    set((state) =>
      state.paymentLoading === loading ? state : { paymentLoading: loading },
    ),
  setPaymentError: (error: string | null) =>
    set((state) =>
      state.paymentError === error ? state : { paymentError: error },
    ),

  // Async actions will be implemented by hooks that use this store
  refreshBillingInfo: async () => {
    // This will be implemented by billing payment hook
    console.warn(
      "refreshBillingInfo should be implemented by billing payment hook",
    );
  ...
  handleSubscription: async () => {
    // This will be implemented by billing payment hook
    console.warn(
      "handleSubscription should be implemented by billing payment hook",
    );
    return Promise.resolve();
  },
