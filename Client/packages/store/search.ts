...
// Placeholder cache utils - replace with actual implementation when available
const cacheUtils = {
...
const baseCreator: import("zustand").StateCreator<ConsolidatedSearchState> = (
  set,
  get,
) => ({
  ...initialState(),
  ...
  loadSearchResultsFromCache: (preferencesVersion = "1.0") => {
    try {
      console.log(
        "📦 [STORE] Loading search results from cache:",
        preferencesVersion,
      );
      const cachedResults =
      ...
      console.error(
        "❌ [STORE] Error loading search results from cache:",
        error,
      );
      return null;
    }
