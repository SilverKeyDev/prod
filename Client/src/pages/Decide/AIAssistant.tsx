...
import { useChats } from "../../core/hooks/data/useChats";
import type { Chat } from "../../core/schemas";
...
export default function AIAssistant() {
  ...
  // Load AI assistant state from sessionStorage on mount (temporary chat state)
  useEffect(() => {
    const savedState = sessionStorage.getItem("aiAssistantState");
    if (savedState) {
      try {
        ...
        sessionStorage.removeItem("aiAssistantState");
      }
    }
  }, []);

  // Save AI assistant state to sessionStorage (temporary chat state)
  useEffect(() => {
    const stateToSave = { activeChatId, message, isTyping };
    sessionStorage.setItem("aiAssistantState", JSON.stringify(stateToSave));
  ...
  // Load chats from centralized context - only update when chats actually change
  useEffect(() => {
    try {
      // Check if chats have actually changed
      const chatsChanged =
        JSON.stringify(chats) !== JSON.stringify(previousChatsRef.current);
      ...
      if (chats && chats.length > 0) {
        // Use functional update to avoid dependency on localChats
        setLocalChats((prevLocalChats) => {
          const updatedChats = chats.map((contextChat: Chat) => {
            const existingChat = prevLocalChats.find(
  ...
  // Refresh on mount (do not depend on refreshChats identity to avoid loops)
  useEffect(() => {
    void refreshChats();
  ...
  return (
    <div className="mx-auto mt-8 h-[calc(100vh-8rem)] max-w-7xl md:mt-0">
      <div className="relative flex h-full overflow-hidden rounded-xl shadow-lg">
        {/* Sidebar (Chat list) */}
        <aside
          className={`flex w-80 flex-col rounded-xl border-r border-beige bg-white transition-transform duration-300 ease-in-out md:relative md:translate-x-0 ${isSidebarExpanded ? "translate-x-0" : "-translate-x-full"} absolute z-40 h-full md:static`}
