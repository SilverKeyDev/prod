...
export const useDocuments = () => {
  ...
  const uploadDocumentMutation = useMutation({
    mutationFn: async ({
      ...
      const newDocument = await documentService.uploadDocument(file, category, propertyId, offerId);
      return newDocument;
    },
    onSuccess: () => {
  ...
  const uploadDocument = useCallback(
    async (
      ...
      const uploadEntry: { id: string; file: File; progress: number; status: string } = {
        id: uploadId,
        file,
        progress: 0,
        status: 'uploading',
