...
export const useMapInitialization = ({
  ...
  useEffect(() => {
    ...
    if (typeof mql.addEventListener === 'function') {
      ...
      typeof (mql as { addListener?: (listener: () => void) => void }).addListener === 'function'
    ) {
      (mql as { addListener: (listener: () => void) => void }).addListener(handleChange);
    }
    ...
    return () => {
      ...
      if (typeof mqlRef.current.removeEventListener === 'function') {
        ...
        typeof (mqlRef.current as { removeListener?: (listener: () => void) => void })
          .removeListener === 'function'
      ) {
        (mqlRef.current as { removeListener: (listener: () => void) => void }).removeListener(
          handleChange
        );
      }
    };
