...
export function useGoogleCalendar(): UseGoogleCalendarReturn {
  ...
  const revokeMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.removeQueries({ queryKey: queryKeys.googleCalendar.all });
    },
  });
...
export function useGoogleEvents(params?: {
  ...
  useEffect(() => {
    if (authReady && isAuthenticated && googleCalendarService.isConnected() && events.length === 0) {
  ...
  const createEventMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      void queryClient.invalidateQueries({ queryKey: queryKeys.googleCalendar.events() });
    },
  });
  ...
  const createEvent = useCallback(async (event: GoogleEvent) => {
    return await createEventMutation.mutateAsync(event);
  }, [createEventMutation]);
