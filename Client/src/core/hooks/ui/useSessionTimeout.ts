...
export function useSessionTimeout(config: SessionTimeoutConfig = {}): SessionTimeoutState & {
  ...
  }, [fullConfig.idleTimeoutMs, sessionStartRef, lastActivityRef, warningShownRef]);
  ...
  useEffect(() => {
    const handleStorageChange = (e: StorageEvent) => {
      ...
      if (
        (e.key === 'id_token' || e.key === 'access_token' || e.key === 'refresh_token') &&
        !e.newValue
      ) {
  ...
  }, [fullConfig.idleTimeoutMs, logout, state.sessionExpired, sessionStartRef, lastActivityRef, warningShownRef]); // Add dependencies for proper cleanup
