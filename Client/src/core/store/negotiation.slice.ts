...
const initialState = (): Omit<NegotiationState, 'setSelectedHome' | 'setStrategyData' | 'setCompsData' | 'setStrategyTextContent' | 'setCompsTextContent' | 'setLoading' | 'setError' | 'clearData' | 'reset'> => ({
  selectedHome: null,
  strategyData: null,
  compsData: null,
  strategyTextContent: '',
...
const withReset = withResettable<NegotiationState>(
  baseCreator,
  (set) => ({
    ...initialState(),
    setSelectedHome: (home) => set({ selectedHome: home }),
...
const withPersist = persistSafe<NegotiationState>(withReset, {
  ...
  migrate: (persisted: unknown) => ({ ...initialState(), ...(persisted as object) }) as NegotiationState,
}) as unknown as import('zustand').StateCreator<NegotiationState>;

const withDev = withDevtools<NegotiationState>('negotiation')(withPersist) as unknown as import('zustand').StateCreator<NegotiationState>;
