...
const baseCreator: import('zustand').StateCreator<AuthState> = (set) => ({
  ...
  logout: () => {
    console.warn('logout should be implemented by useAuthStoreIntegration hook');
  },
  refreshToken: () => {
    console.warn('refreshToken should be implemented by useAuthStoreIntegration hook');
    return Promise.resolve(false);
  },
  clearError: () => {
    set({ error: null });
...
const withReset = withResettable<AuthState>(
  baseCreator,
  (set) => ({
    ...initialState(),
    setUser: (user) => set({ user }),
...
const withPersist = persistSafe<AuthState>(withReset, {
  ...
  migrate: (persisted: unknown, _version: number): AuthState => {
    ...
    if (!persisted) return { ...base, reset: () => {}, setUser: base.setUser as any, setIsAuthenticated: base.setIsAuthenticated as any, setIsLoading: base.setIsLoading as any, setError: base.setError as any, setAuthReady: base.setAuthReady as any, login: base.login as any, logout: base.logout as any, refreshToken: base.refreshToken as any, clearError: base.clearError as any } as unknown as AuthState;
...
const withDev = withDevtools<AuthState>('auth')(withPersist) as unknown as import('zustand').StateCreator<AuthState>;
