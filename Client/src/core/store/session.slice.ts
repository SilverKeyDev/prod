...
const withReset = withResettable<SessionState>(
  baseCreator,
  (set) => ({
    ...initialState(),
    setAuthReady: (ready) => set({ authReady: ready }),
...
const withPersist = persistSafe<SessionState>(withReset, {
  ...
  migrate: (persisted: unknown, _version: number): SessionState => {
    ...
    if (!persisted) return { ...base, reset: () => {}, softReset: () => {}, setAuthReady: base.setAuthReady as any, setIsAuthenticated: base.setIsAuthenticated as any, setUserMeta: base.setUserMeta as any, setFeatureGates: base.setFeatureGates as any } as unknown as SessionState;
...
const withDev = withDevtools<SessionState>('session')(withPersist) as unknown as import('zustand').StateCreator<SessionState>;
