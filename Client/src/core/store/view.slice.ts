...
const baseCreator: import('zustand').StateCreator<ViewState> = (set) => ({
  ...
  setCategoryOpen: (category, isOpen) =>
    set((s) => ({ openCategories: { ...s.openCategories, [category]: isOpen } })),
  toggleCategory: (category) =>
    set((s) => ({ openCategories: { ...s.openCategories, [category]: !s.openCategories[category] } })),

  setDropdownSelection: (key, value) =>
    set((s) => ({ dropdownSelections: { ...s.dropdownSelections, [key]: value } })),
...
const withReset = withResettable<ViewState>(
  baseCreator,
  (set) => ({
    ...initialState(),
    setSidebarExpanded: (expanded) => set({ sidebarExpanded: expanded }),
...
const withDev = withDevtools<ViewState>('view')(withPersist) as unknown as import('zustand').StateCreator<ViewState>;
