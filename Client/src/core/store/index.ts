...
export const selectIsAuthenticated = (s: unknown): boolean =>
  !!(s && typeof s === 'object' && 'isAuthenticated' in s && (s as Record<string, unknown>).isAuthenticated);
...
export const selectUserMeta = (s: unknown): unknown =>
  s && typeof s === 'object' && 'userMeta' in s ? (s as Record<string, unknown>).userMeta : undefined;

export const selectFeatureFlags = (s: unknown): Record<string, boolean> =>
  s && typeof s === 'object' && 'flags' in s && typeof (s as Record<string, unknown>).flags === 'object'
