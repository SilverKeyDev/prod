...
const withReset = withResettable<UIState>(
  baseCreator,
  (set) => ({
    ...initialState(),
    openModal: () => set({ isAnyModalOpen: true }),
    ...
    setCarouselCollapsed: (collapsed) => set({ isCarouselCollapsed: collapsed }),
    setShowPropertyModals: (show) => set({ showPropertyModals: show }),
    enqueueToast: (toast) => set((state: UIState) => {
      const id = toast.id ?? `${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
      const nextQueue = [...state.toastQueue, { id, message: toast.message, type: toast.type }];
      return { toastQueue: nextQueue, activeToastId: state.activeToastId ?? id } as Partial<UIState> as UIState;
    }),
    dequeueToast: (id) => set((state: UIState) => {
      const targetId = id ?? state.activeToastId ?? state.toastQueue[0]?.id;
      const nextQueue = state.toastQueue.filter((t) => t.id !== targetId);
      const nextActive = nextQueue[0]?.id ?? null;
      return { toastQueue: nextQueue, activeToastId: nextActive } as Partial<UIState> as UIState;
    }),
    clearToasts: () => set({ toastQueue: [], activeToastId: null }),
    reset: () => {},
  })
...
const withDev = withDevtools<UIState>('ui')(withPersist) as unknown as import('zustand').StateCreator<UIState>;
