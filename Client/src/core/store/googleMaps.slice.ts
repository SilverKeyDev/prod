...
export type GoogleMapsState = {
  ...
  getServiceState: () => { isLoaded: boolean; isLoading: boolean; error: string | null; scriptUrl: string | null };
  
  // Reset
  reset: () => void;
};

const initialState = (): Omit<GoogleMapsState, 'setLoaded' | 'setLoading' | 'setError' | 'setScriptUrl' | 'loadGoogleMaps' | 'createMap' | 'getServiceState' | 'reset'> => ({
  isLoaded: false,
  isLoading: false,
  error: null,
  scriptUrl: null,
...
const withReset = withResettable<GoogleMapsState>(
  baseCreator,
  (set, get) => ({
    ...initialState(),
    setLoaded: (loaded) => set({ isLoaded: loaded }),
...
const withPersist = persistSafe<GoogleMapsState>(withReset, {
  ...
  migrate: (persisted: unknown) => ({ ...initialState(), ...(persisted as object) }) as GoogleMapsState,
}) as unknown as import('zustand').StateCreator<GoogleMapsState>;

const withDev = withDevtools<GoogleMapsState>('google-maps')(withPersist) as unknown as import('zustand').StateCreator<GoogleMapsState>;
