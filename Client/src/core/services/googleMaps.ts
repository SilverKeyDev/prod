...
export class GoogleMapsService {
  ...
  private preloadLibraries(): void {
    if (window.google?.maps?.importLibrary) {
      ...
      window.google.maps
        ...
        .catch((err: unknown) => console.warn('Failed to import marker library:', err));

      // Preload places library for geocoding functionality
      window.google.maps
        .importLibrary('places')
        .catch((err: unknown) => console.warn('Failed to import places library:', err));
    }
  }
  ...
  public async loadGoogleMapsScript(): Promise<void> {
    ...
    if (this.hasExistingScripts()) {
      try {
        ...
        console.error('🗺️ [GMAPS_SERVICE] Failed to wait for existing script:', error);
        throw error;
      }
    }
    ...
    this.loadPromise = (async () => {
      try {
        ...
        if (!data.success || !data.script_url) {
          ...
          console.error('🗺️ [GMAPS_SERVICE] ❌ Failed to get script URL:', errorMsg);
          this.error = errorMsg;
        ...
        console.error('🗺️ [GMAPS_SERVICE] ❌ Error loading Google Maps:', errorMsg);
        this.error = errorMsg;
        throw err;
      } finally {
        this.isLoading = false;
  ...
  private loadScript(scriptUrl: string): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      script.onerror = (error) => {
        console.error('🗺️ [GMAPS_SERVICE] ❌ Failed to load Google Maps script:', error);
  ...
  public createMap(container: HTMLElement): google.maps.Map | null {
    console.log('🔍 [GMAPS_SERVICE] createMap called with:', {
      ...
      windowGoogleMapsMapTypeControlStyle: typeof window.google?.maps?.MapTypeControlStyle !== 'undefined',
      windowGoogleMapsGeocoder: !!window.google?.maps?.Geocoder
    });

    if (!this.isGoogleMapsReady()) {
      console.error('🗺️ [GMAPS_SERVICE] Google Maps not ready yet - missing required APIs');
      return null;
    }
