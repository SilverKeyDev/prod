...
import type { HomeUniversal, AddFavoriteHomeRequest, RemoveFavoriteHomeRequest, FavoriteHomeResponse } from '../schemas/api';
...
const mapToAddFavoriteHomePayload = (input: unknown): AddFavoriteHomeRequest['home'] => {
  ...
  const getNumber = (v: unknown, fallback = 0): number => (typeof v === 'number' ? v : fallback);
  ...
  const imageUrl = obj.imageUrl !== undefined ? getString(obj.imageUrl) : getString(obj.image_url, undefined as unknown as string);
...
export class SavedHomesService {
  ...
  public async fetchSavedHomes(): Promise<SavedHome[]> {
    ...
    try {
      ...
      if (typedResponse && typeof typedResponse === 'object' && 'success' in typedResponse && typedResponse.success) {
        const rawHomes = typedResponse && typeof typedResponse === 'object' && 'favorites' in typedResponse && Array.isArray(typedResponse.favorites) ? typedResponse.favorites : [];
        const homeObjects: SavedHome[] = rawHomes.map(mapHomeUniversalToSavedHome);
        return homeObjects;
      } else {
        const errorMsg = typedResponse && typeof typedResponse === 'object' && 'error' in typedResponse && typeof typedResponse.error === 'string' ? typedResponse.error : 'Failed to load favorite homes';
        if (log && typeof log.error === 'function') {
          log.error('SAVED_HOMES_SERVICE', 'Failed to fetch saved homes', { error: errorMsg });
        }
  ...
  public async saveHome(property: unknown): Promise<{ success: boolean; error?: string }> {
    try {
      const request: AddFavoriteHomeRequest = { home: mapToAddFavoriteHomePayload(property) };
      ...
      if (typedResponse && typeof typedResponse === 'object' && 'success' in typedResponse && typedResponse.success) {
        ...
        const errorMsg = typedResponse && typeof typedResponse === 'object' && 'error' in typedResponse && typeof typedResponse.error === 'string' ? typedResponse.error : 'Failed to save home';
        if (log && typeof log.error === 'function') {
          log.error('SAVED_HOMES_SERVICE', 'Failed to save home', { error: errorMsg });
        }
        return {
          success: false,
          error: errorMsg,
  ...
  public async removeSavedHome(address: string): Promise<{ success: boolean; error?: string }> {
    try {
      ...
      if (typedResponse && typeof typedResponse === 'object' && 'success' in typedResponse && typedResponse.success) {
        ...
        const errorMsg = typedResponse && typeof typedResponse === 'object' && 'error' in typedResponse && typeof typedResponse.error === 'string' ? typedResponse.error : 'Failed to remove home';
        if (log && typeof log.error === 'function') {
          log.error('SAVED_HOMES_SERVICE', 'Failed to remove home', { error: errorMsg });
        }
        return {
          success: false,
          error: errorMsg,
