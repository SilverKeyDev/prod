...
import { buildApiUrl, apiDownloadBlob } from '../services/http/compatibility';
...
export class PlaidService {
   ...
   */
  public async createLinkToken(request: CreateLinkTokenRequest = {}): Promise<ApiResponse<PlaidLinkToken>> {
    try {
      log.info('PLAID_SERVICE', 'Creating link token', { products: request.products });
      
      const response = await apiPost<ApiResponse<PlaidLinkToken>>(ENDPOINTS.LINK_TOKEN_CREATE, request);
      
      if (response.success) {
        log.info('PLAID_SERVICE', 'Link token created successfully');
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Link token creation failed', { error: response.error });
   ...
   */
  public async exchangeToken(request: ExchangeTokenRequest): Promise<ApiResponse<{ item_id: string }>> {
    try {
      log.info('PLAID_SERVICE', 'Exchanging public token');
      
      const response = await apiPost<ApiResponse<{ item_id: string }>>(ENDPOINTS.TOKEN_EXCHANGE, request);
      
      if (response.success) {
        log.info('PLAID_SERVICE', 'Token exchanged successfully', { itemId: response.data?.item_id });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Token exchange failed', { error: response.error });
  ...
  public async getItems(): Promise<ApiResponse<PlaidItem[]>> {
    try {
      ...
      if (response.success) {
        log.info('PLAID_SERVICE', 'Plaid items fetched successfully', { count: response.data?.length });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Failed to fetch Plaid items', { error: response.error });
  ...
  public async disconnectItem(itemId: string): Promise<ApiResponse<void>> {
    try {
      ...
      if (response.success) {
        log.info('PLAID_SERVICE', 'Plaid item disconnected successfully', { itemId });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Failed to disconnect Plaid item', { itemId, error: response.error });
      ...
      if (!isAbortError(error)) {
        ...
        log.error('PLAID_SERVICE', 'Plaid item disconnect error', { itemId, error });
      }
      throw error;
    }
  }
   ...
   */
  public async createAssetReport(request: CreateAssetReportRequest = {}): Promise<ApiResponse<{ asset_report_token: string }>> {
    try {
      log.info('PLAID_SERVICE', 'Creating asset report', { days: request.days_requested });
      
      const response = await apiPost<ApiResponse<{ asset_report_token: string }>>(ENDPOINTS.ASSET_REPORT_CREATE, request);
      
      if (response.success) {
        log.info('PLAID_SERVICE', 'Asset report created successfully', { token: response.data?.asset_report_token });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Asset report creation failed', { error: response.error });
   ...
   */
  public async getAssetReport(assetReportToken: string): Promise<ApiResponse<PlaidAssetReportData>> {
    try {
      log.info('PLAID_SERVICE', 'Fetching asset report', { token: assetReportToken });
      
      const url = buildApiUrl(ENDPOINTS.ASSET_REPORT_GET, { asset_report_token: assetReportToken });
      ...
      if (response.success) {
        ...
        log.warn('PLAID_SERVICE', 'Failed to fetch asset report', { error: response.error });
  ...
  public async getAssetReportPdf(assetReportToken: string): Promise<Blob> {
    try {
      log.info('PLAID_SERVICE', 'Downloading asset report PDF', { token: assetReportToken });
      
      const url = buildApiUrl(ENDPOINTS.ASSET_REPORT_PDF, { asset_report_token: assetReportToken });
      ...
      if (!isAbortError(error)) {
        ...
        log.error('PLAID_SERVICE', 'Asset report PDF download error', { error });
      }
      throw error;
    }
  }
  ...
  public async getAssetReports(): Promise<ApiResponse<PlaidAssetReport[]>> {
    try {
      ...
      const response = await apiGet<ApiResponse<PlaidAssetReport[]>>(ENDPOINTS.ASSET_REPORT_GET);
      
      if (response.success) {
        log.info('PLAID_SERVICE', 'Asset reports fetched successfully', { count: response.data?.length });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Failed to fetch asset reports', { error: response.error });
  ...
  public async getStatements(): Promise<ApiResponse<PlaidStatement[]>> {
    try {
      ...
      const response = await apiGet<ApiResponse<PlaidStatement[]>>(ENDPOINTS.STATEMENTS);
      
      if (response.success) {
        log.info('PLAID_SERVICE', 'Bank statements fetched successfully', { count: response.data?.length });
        return response;
      } else {
        log.warn('PLAID_SERVICE', 'Failed to fetch bank statements', { error: response.error });
  ...
  public async downloadStatement(statementId: string): Promise<Blob> {
    try {
      ...
      const url = buildApiUrl(ENDPOINTS.STATEMENT_DOWNLOAD, { statement_id: statementId });
      const blob = await apiDownloadBlob(url);
...
export const plaidApi = {
  createLinkToken: (request: CreateLinkTokenRequest = {}) => plaidService.createLinkToken(request),
  exchangeToken: (request: ExchangeTokenRequest) => plaidService.exchangeToken(request),
  getItems: () => plaidService.getItems(),
  disconnectItem: (itemId: string) => plaidService.disconnectItem(itemId),
  createAssetReport: (request: CreateAssetReportRequest = {}) => plaidService.createAssetReport(request),
  getAssetReport: (assetReportToken: string) => plaidService.getAssetReport(assetReportToken),
  getAssetReportPdf: (assetReportToken: string) => plaidService.getAssetReportPdf(assetReportToken),
  getAssetReports: () => plaidService.getAssetReports(),
  getStatements: () => plaidService.getStatements(),
  downloadStatement: (statementId: string) => plaidService.downloadStatement(statementId),
};
