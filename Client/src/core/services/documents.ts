    ...
    try {
      ...
      if (response.success && response.documents) {
        ...
        return documents.filter(isDocumentData).map((doc: unknown) => {
          ...
          return {
            id: typeof docData.id === 'string' || typeof docData.id === 'number' ? String(docData.id) : '',
            name: typeof docData.filename === 'string' || typeof docData.filename === 'number' ? String(docData.filename) : '',
            file_path: typeof docData.file_path === 'string' || typeof docData.file_path === 'number' ? String(docData.file_path) : '',
            ...
            offer_id: undefined,
            uploaded_by: typeof docData.user_id === 'string' || typeof docData.user_id === 'number' ? String(docData.user_id) : '',
            uploaded_at: new Date(typeof docData.created_at === 'string' || typeof docData.created_at === 'number' ? String(docData.created_at) : ''),
            is_signed: false,
            expiry_date: undefined,
            status: String(docData.status) === 'completed' ? 'approved' : 'pending',
          };
        });
      } else {
        return [];
