...
import { reportSecurityEvent, clearUserContext } from './security/errorReporting';
...
export const authService = {
  ...
  logout: (): LogoutResult => {
    try {
      ...
      const windowWithSecureTokens = window as unknown as { clearSecureTokens?: () => void };
  ...
  isAuthenticated: (): boolean => {
    ...
    const sessionToken = sessionStorage.getItem(AUTH_CONFIG.STORAGE_KEYS.ACCESS_TOKEN);
