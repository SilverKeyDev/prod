...
class ErrorReporter {
  ...
  constructor() {
    ...
    const env = import.meta?.env && typeof import.meta.env === 'object' ? import.meta.env : {};
    this.isProduction = 'MODE' in env && env.MODE === 'production';
    this.buildVersion = 'VITE_BUILD_VERSION' in env && typeof env.VITE_BUILD_VERSION === 'string' 
  ...
  private setupGlobalErrorHandlers(): void {
    ...
    window.addEventListener('react-error', (event: unknown) => {
      if (event && typeof event === 'object' && 'detail' in event) {
        const customEvent = event as { detail: { error: Error; componentStack: string } };
