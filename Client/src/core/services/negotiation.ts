...
export class NegotiationService {
  ...
  private constructor() {
    this.state = {
      selectedHome: this.loadFromLocalStorage(this.localStorageKeys.selectedHome),
  ...
  public selectHome(home: unknown): void {
    log.info('NEGOTIATION_SERVICE', 'Home selected', { 
      homeAddress: home && typeof home === 'object' && 'address' in home && typeof home.address === 'string' ? home.address : undefined 
    });
  ...
  public async generateStrategy(): Promise<void> {
    ...
    try {
      ...
      if (isObject(this.state.selectedHome)) {
        const { address: homeAddress } = this.state.selectedHome as { address?: string };
        address = homeAddress;
      } else if (hasProperty(this.state.selectedHome, 'full_address') && typeof (this.state.selectedHome as Record<string, unknown>).full_address === 'string') {
        const homeData = this.state.selectedHome as Record<string, unknown>;
        address = homeData.full_address as string;
      } else if (hasProperty(this.state.selectedHome, 'location') && typeof (this.state.selectedHome as Record<string, unknown>).location === 'string') {
        const homeData = this.state.selectedHome as Record<string, unknown>;
        address = homeData.location as string;
      } else {
        address = this.state.selectedHome ? (typeof this.state.selectedHome === 'string' ? this.state.selectedHome : JSON.stringify(this.state.selectedHome)) : '';
      }
      ...
      if (compsResponseData && typeof compsResponseData === 'object' && 'success' in compsResponseData && !compsResponseData.success) {
        if (log && typeof log.warn === 'function') {
          log.warn('NEGOTIATION_SERVICE', 'Property comps API failed', {
            error: 'error' in compsResponseData && typeof compsResponseData.error === 'string' 
      ...
      const parsedStrategyData = 'strategy' in strategyResponse && strategyResponse.strategy 
        ? strategyResponse.strategy as Record<string, unknown>
        : {};
      log.info('NEGOTIATION_SERVICE', 'Strategy generated successfully', {
        strategyId: 'strategy_id' in strategyResponse && typeof strategyResponse.strategy_id === 'string' 
          ? strategyResponse.strategy_id 
          : undefined,
      });

      // Extract textual content from strategy data
      const strategyTextContent = this.extractTextContent(parsedStrategyData ?? {});
      ...
      // Persist to localStorage
      this.saveToLocalStorage(this.localStorageKeys.strategy, parsedStrategyData ?? {});
      this.saveToLocalStorage(this.localStorageKeys.comps, compsResponseData ?? {});
    ...
    const extractFromObject = (obj: Record<string, unknown>, depth = 0): void => {
      ...
      for (const [key, value] of Object.entries(obj)) {
        if (typeof value === 'string' && value.trim().length > 0) {
          ...
          if (value.length > 3 && !['id', 'type', 'status', 'created_at', 'updated_at'].includes(key.toLowerCase())) {
  ...
  public downloadStrategyJson(): void {
    ...
    try {
      ...
      const url = typeof URL !== 'undefined' && URL.createObjectURL ? URL.createObjectURL(dataBlob) : null;
      ...
      const address = isObject(this.state.selectedHome)
        ? (this.state.selectedHome && typeof this.state.selectedHome === 'object' && 'address' in this.state.selectedHome && typeof this.state.selectedHome.address === 'string' ? this.state.selectedHome.address : '')
        : (this.state.selectedHome ? (typeof this.state.selectedHome === 'string' ? this.state.selectedHome : JSON.stringify(this.state.selectedHome)) : '');
