...
export class ReportsService {
  ...
  public async fetchAllReportsData(): Promise<unknown> {
    try {
      ...
      if (response.success && response.reports) {
        ...
        (window as unknown as { sharedReportsData: unknown }).sharedReportsData = null;
  ...
  public async generateReport(data: {
    ...
    try {
      ...
      if (response.success) {
        ...
        log.error('REPORTS_SERVICE', 'Report generation failed', { error: errorMsg });
        return {
          success: false,
          error: errorMsg,
        };
  ...
  public async deleteReport(
    ...
    try {
      ...
      if (response.success) {
        ...
        log.error('REPORTS_SERVICE', 'Report deletion failed', { error: errorMsg });
        return {
          success: false,
          error: errorMsg,
        };
  ...
  public async compareReports(
    ...
    try {
      ...
      const response = await reportApi.compare({ report_ids: reportIds, s3Keys: normalizedKeys });
      ...
      if (response.success) {
        ...
        log.error('REPORTS_SERVICE', 'Report comparison failed', { error: errorMsg });
        return {
          success: false,
          error: errorMsg,
        };
  ...
  public async getDownloadUrl(reportId: string): Promise<{
    ...
    try {
      ...
      if (response.success) {
        ...
        log.error('REPORTS_SERVICE', 'Download URL retrieval failed', { error: errorMsg });
        return {
          success: false,
          error: errorMsg,
        };
  ...
  public async getViewUrl(reportId: string): Promise<{
    ...
    try {
      ...
      if (response.success) {
        ...
        log.error('REPORTS_SERVICE', 'View URL retrieval failed', { error: errorMsg });
        return {
          success: false,
          error: errorMsg,
        };
      ...
      return {
        success: false,
        error: error instanceof Error ? error.message : 'Failed to get view URL',
      };
    }
  }
