...
export class ChatService {
  ...
  async fetchChats(): Promise<Chat[]> {
    ...
    try {
      ...
      if (sharedData && Date.now() - sharedData.timestamp < CACHE_TTL) {
        console.log('[CHAT_SERVICE] 📋 Using shared reports data from ReportsContext');
        ...
        const newChats: Chat[] = reports.map((report: unknown) => {
          ...
          return {
            id: typeof reportData.id === 'string' ? reportData.id : (typeof reportData.id === 'number' ? String(reportData.id) : 'unknown'),
            title: typeof reportData.address === 'string'
            ...
            messages: [],
            createdAt: new Date(
              typeof reportData.generatedAt === 'number' ? reportData.generatedAt * 1000 : Date.now()
            ),
          };
        });

        console.log('[CHAT_SERVICE] ✅ Successfully processed chats from shared data:', {
      ...
      console.log('[CHAT_SERVICE] 📡 No shared data available, calling reportApi.getAll()');
      const json = await reportApi.getAll();
      console.log('[CHAT_SERVICE] 📥 API Response received:', {
        success: json && typeof json === 'object' && 'success' in json ? json.success : false,
        reportsCount: json && typeof json === 'object' && 'reports' in json && Array.isArray(json.reports) ? json.reports.length : 0,
        hasReports: json && typeof json === 'object' && 'reports' in json && Array.isArray(json.reports),
        error: json && typeof json === 'object' && 'error' in json ? json.error : undefined,
      });

      if (json && typeof json === 'object' && 'success' in json && json.success && 'reports' in json && Array.isArray(json.reports)) {
        const reports = json && typeof json === 'object' && 'reports' in json && Array.isArray(json.reports) ? json.reports : [];
        const newChats: Chat[] = reports.map((report: unknown) => {
          if (!report || typeof report !== 'object') {
            throw new Error('Invalid report data structure');
          }
        ...
        const errorMsg = json && typeof json === 'object' && 'error' in json ? json.error : 'Failed to fetch chat data';
      ...
      if (!isAbortError(e)) {
        console.error('[CHAT_SERVICE] ❌ fetchChats error:', {
          error: e,
          message: e && typeof e === 'object' && 'message' in e && typeof e.message === 'string' ? e.message : undefined,
          stack: e && typeof e === 'object' && 'stack' in e && typeof e.stack === 'string' ? e.stack : undefined,
        });
        throw e;
      }
      throw e;
  ...
  async sendMessage(reportId: string, message: string): Promise<unknown> {
    ...
    try {
      const cleanReportId = typeof reportId === 'string' ? reportId.replace(/\.(pdf|json)$/, '') : String(reportId);
      ...
      console.log('[CHAT_SERVICE] ✅ sendMessage response:', {
        hasResponse: response && typeof response === 'object' && 'response' in response && typeof response.response === 'string',
        messageId: response && typeof response === 'object' && 'message_id' in response && typeof response.message_id === 'string' ? response.message_id : undefined,
        messageSummary: response && typeof response === 'object' && 'message_summary' in response && typeof response.message_summary === 'string' ? response.message_summary : undefined,
      });
      ...
      console.error('[CHAT_SERVICE] ❌ sendMessage error:', {
        reportId,
        cleanReportId: typeof reportId === 'string' ? reportId.replace(/\.(pdf|json)$/, '') : String(reportId),
        error,
  ...
  async getChatHistory(reportId: string): Promise<unknown> {
    ...
    try {
      ...
      console.log('[CHAT_SERVICE] ✅ getChatHistory response:', {
        messagesCount: 'messages' in typedResponse && Array.isArray(typedResponse.messages) ? typedResponse.messages.length : 0,
