import { apiGet, apiPost, buildApiUrl } from '../../services/http/compatibility';
...
export const searchApi = {
    ...
    return apiGet<PropertyCompsResponse>(url, {
      ...
      .then((resp) => {
        console.log('✅ [searchApi.getPropertyComps] Response', {
          ...
          compsCount: Array.isArray((resp as any)?.comps) ? (resp as any).comps.length : undefined,
          hasError: !!resp?.error,
        });
        return resp;
      })
      .catch((error) => {
        console.error('❌ [searchApi.getPropertyComps] Error', { message: String(error) });
        throw error;
      });
  },
  ...
  getProperty: (data: PropertyRequest): Promise<PropertyResponse> => {
    ...
    return apiPost<PropertyResponse>(url, data, {
      ...
      .catch((error) => {
        console.error('❌ [searchApi.getProperty] Error', { message: String(error) });
        throw error;
      });
  },
  ...
  searchByPolygon: (data: PolygonSearchRequest): Promise<PolygonSearchResponse> => {
    ...
    return apiPost<PolygonSearchResponse>(url, data, {
      ...
      .catch((error) => {
        console.error('❌ [searchApi.searchByPolygon] Error', { message: String(error) });
        throw error;
      });
  },
  ...
  getIsochrone: (): Promise<IsochroneResponse> => {
    ...
    return apiGet<IsochroneResponse>(url, {
      ...
      .catch((error) => {
        console.error('❌ [searchApi.getIsochrone] Error', { message: String(error) });
        throw error;
      });
  },
};
