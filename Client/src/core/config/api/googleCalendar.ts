...
export const googleCalendarApi = {
  ...
  revokeAccess: async (): Promise<GoogleCalendarApiResponse<{ ok: boolean }>> => {
    try {
      const response = await apiPost('/api/v1/google/oauth/revoke', {});
      return response;
    } catch (error) {
      return {
        success: false,
  ...
  listCalendars: async (): Promise<GoogleCalendarApiResponse<GoogleCalendarListResponse>> => {
    try {
      const response = await apiGet('/api/v1/google/me/calendars');
      return response;
    } catch (error) {
      return {
        success: false,
   ...
   * Create a new event
   */
  createEvent: async (event: GoogleEvent): Promise<GoogleCalendarApiResponse<GoogleEventCreateResponse>> => {
    try {
      const response = await apiPost('/api/v1/google/me/events', event);
      return response;
    } catch (error) {
      return {
        success: false,
