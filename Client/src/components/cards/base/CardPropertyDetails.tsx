...
const CardPropertyDetails: React.FC<CardPropertyDetailsProps> = ({
  ...
  const calculateOptimalSize = () => {
    const shouldShowSqft =
      showSquareFootage && !hideSquareFootage && sqft && sqft > 0;
    const contentCount = [
      bedrooms,
      bathrooms,
      shouldShowSqft ? sqft : undefined,
  ...
  const layoutStyles = {
    horizontal: "flex items-center justify-start flex-nowrap",
    vertical: "flex items-center justify-start flex-nowrap",
    grid: "flex items-center justify-start flex-nowrap",
    ...
    <div className={containerClasses}>
      {bedrooms !== undefined && (
        <div className="flex flex-shrink-0 items-center">
          ...
          <span
            className={`${currentSizeStyles.text} whitespace-nowrap text-gray-600`}
          >
            {variant === "modal"
              ? bedrooms
