  ...
  useEffect(() => {
    const calculateMaxHeights = () => {
      ...
      if (items && items.length > 0) {
        ...
        const titleElements =
          containerRef.current.querySelectorAll<HTMLElement>('.aligned-row-title');
        ...
        const titleHeights = Array.from(titleElements).map((el) => el.offsetHeight);
        const contentHeights = Array.from(contentElements).map((el) => el.offsetHeight);

        const newMaxTitleHeight = titleHeights.length > 0 ? Math.max(...titleHeights) : 0;
        const newMaxContentHeight = contentHeights.length > 0 ? Math.max(...contentHeights) : 0;
        ...
        const finalHeight = minHeight
          ? Math.max(newMaxHeight, typeof minHeight === 'string' ? parseInt(minHeight) : minHeight)
          : newMaxHeight;
  ...
  const renderContent = () => {
    if (items && items.length > 0) {
      return items.map((item, index) => {
        ...
        return (
          <div
            key={index}
            className={`aligned-row-item flex h-full min-h-0 flex-col ${responsiveWidthClasses} ${
              item.className ?? ''
            }`}
            style={widthStyle}
          >
    ...
    if (children) {
      return React.Children.map(children, (child, index) => {
        ...
        const childElement = React.isValidElement(child) ? child : <div>{child}</div>;
        return (
          <div
            key={index}
            className={`aligned-row-item flex h-full min-h-0 flex-col ${responsiveWidthClasses}`}
            style={widthStyle}
          >
            {React.cloneElement(childElement as React.ReactElement<{ className?: string }>, {
              className: `${
                (childElement as React.ReactElement<{ className?: string }>)?.props?.className ?? ''
