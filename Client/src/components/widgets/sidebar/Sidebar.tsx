...
const navigationStructure: NavigationStructure = {
  ...
  calendar: {
    name: "Calendar",
    icon: Calendar,
    items: [{ name: "Calendar", href: "/calendar", icon: Calendar }],
  },
  onboard: {
    name: "Onboard",
    icon: ClipboardList,
...
export default function Sidebar({
  ...
  const { userProfile, userLoading: userProfileLoading } = useUserStore();
            ...
            <nav className="mt-4 pb-4">
              {Object.entries(getNavigation()).map(
                ([categoryKey, category]: [string, NavCategory]) => (
                  <div key={categoryKey}>
                    {/* Render certain categories as direct links (search, dashboard, calendar) */}
                    ...
                    categoryKey === "calendar" ? (
                      <Link
                        ...
                        onClick={() => {
                          console.log(
                            `[SIDEBAR] ${categoryKey === "dashboard" ? "🏠" : "📅"} ${category.name} navigation clicked:`,
                            {
                        ...
                        className={`${getButtonStyles(isActive(category.items[0]?.href ?? "/"))} ${
