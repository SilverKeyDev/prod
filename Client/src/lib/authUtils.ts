...
import { fetchJson, logHttp, createAuthHeaders, isAuthenticationError, handleAuthenticationError } from '../api/utils/index';
...
export function useAuthState(): AuthState {
  ...
  const { value: idToken, removeValue: removeIdToken } = useLocalStorage<string | null>('id_token', null);
  const { removeValue: removeAccessToken } = useLocalStorage<string | null>('access_token', null);
  ...
  const checkAuth = useCallback(async () => {
    ...
    console.log('🔒 [AUTH] Checking authentication - secureToken:', !!secureToken, 'sessionToken:', !!sessionToken, 'idToken:', !!idToken);
    ...
    try {
      ...
      if (response?.success && response?.data) {
        console.log('🔒 [AUTH] Token valid, user authenticated:', response.data.email || 'unknown');
  ...
  useEffect(() => {
    ...
    if (import.meta.env.DEV && didRun.current && refreshTrigger === 0) {
      console.log('🔒 [AUTH] Skipping duplicate auth check due to StrictMode double-mount');
      return;
    }
    didRun.current = true;
    
    console.log('🔒 [AUTH] Initializing authentication check (trigger:', refreshTrigger, ')');
    checkAuth();
