...
export function useSessionTimeout(
  ...
  useEffect(() => {
    ...
    fullConfig.idleTimeoutMs,
    fullConfig.maxSessionMs,
    fullConfig.warningTimeMs,
    fullConfig.checkIntervalMs,
    logout,
  ]);
  ...
  useEffect(() => {
    ...
    const handleAuthChange = () => {
      ...
      const checkAuthState = () => {
        ...
        console.log(
          "[SESSION_TIMEOUT] 🔄 Auth change detected (attempt " +
            attempts +
            "):",
          {
            hasUser: !!hasUser,
        ...
        if (!hasUser && hasAccessToken && attempts < maxAttempts) {
          console.log(
            "[SESSION_TIMEOUT] ⏳ Waiting for localStorage user data...",
          );
        ...
        if (!hasUser && !hasAccessToken) {
          console.log(
            "[SESSION_TIMEOUT] ❌ No user or tokens found, triggering logout",
          );
          logout();
        } else if (hasUser) {
          // User logged in - reset session timers
          console.log(
            "[SESSION_TIMEOUT] ✅ User found, resetting session timers",
          );
          sessionStartRef.current = Date.now();
          lastActivityRef.current = Date.now();
          warningShownRef.current = false;
          ...
          console.log(
            "[SESSION_TIMEOUT] ⚠️ Have tokens but no user data after max attempts - assuming login in progress",
          );
