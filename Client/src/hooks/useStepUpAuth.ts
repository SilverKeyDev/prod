...
interface UseStepUpAuthReturn {
  ...
  requestStepUpAuth: (action: string, description?: string, config?: StepUpAuthConfig) => Promise<boolean>;
...
export const useStepUpAuth = (): UseStepUpAuthReturn => {
  ...
  const { value: stepUpCache, setValue: setStepUpCache } = useLocalStorage<Record<string, number>>('stepUpAuthCache', {});

  /**
   * Check if user has recently authenticated for this action
   */
  const isRecentlyAuthenticated = useCallback((action: string): boolean => {
   ...
   */
  const isStepUpRequired = useCallback((action: string): boolean => {
  ...
  const requestStepUpAuth = useCallback(
    (action: string, description?: string, config: StepUpAuthConfig = {}): Promise<boolean> => {
      return new Promise((resolve) => {
        log.security('STEP_UP_AUTH', 'Step-up authentication requested', { action });
  ...
  const handleSuccess = useCallback(() => {
    log.security('STEP_UP_AUTH', 'Step-up authentication completed successfully', { 
  ...
  const handleClose = useCallback(() => {
    log.info('STEP_UP_AUTH', 'Step-up authentication cancelled', { action: currentAction });
 ...
 */
export const withStepUpAuth = <T extends any[]>(
...
export const checkStepUpRequired = (action: string): boolean => {
  if (SENSITIVE_ACTIONS.includes(action as SensitiveAction)) {
    try {
      const cacheData = localStorage.getItem('stepUpAuthCache');
      const stepUpCache = cacheData ? JSON.parse(cacheData) : {};
      const authTime = stepUpCache[action];
