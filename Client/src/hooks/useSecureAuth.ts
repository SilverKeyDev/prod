...
export interface UseSecureAuthReturn extends SecureAuthState, SecureAuthActions {}
  ...
  const login = useCallback(async (email: string, password: string): Promise<boolean> => {
    ...
    try {
      ...
      if (response.success && response.access_token) {
        ...
        log.security('SECURE_AUTH', 'Login successful with secure token storage', { 
          email,
...
export const getSecureAccessToken = (): string | null => {
  ...
  return sessionStorage.getItem('access_token') || localStorage.getItem('id_token');
};
...
export const secureTokenUtils = {
   ...
   */
  storeTokens: (tokens: { access_token?: string; refresh_token?: string; id_token?: string }) => {
