...
| Path | Purpose |
| ---- | ------- |
| `rules/*.md` | Rule adapters — `@` pointers to `.cursor/rules/**/*.mdc` (same layout as [`.codex/rules/`](../.codex/rules/)) |
...
Also mirrored for OpenAI Codex: [`.codex/`](../.codex/) + [`.agents/skills/`](../.agents/skills/).
...
2. If you add/rename a **scoped** rule, add/update matching **`rules/<flat-name>.md`** here and in **`.codex/rules/`** with `alwaysApply: false` + CSV `paths:` (see [rules/README.md](rules/README.md)). **Never add pathless stubs** for the seven always-on rules — those load only via `CLAUDE.md` `@` includes.
