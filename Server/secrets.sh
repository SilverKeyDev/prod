...
{
  echo "# Auto-generated by secrets.sh on $stamp"
  echo "# Region: $REGION"
  echo
} > "$tmp_env"
{
  echo "# Regenerated by secrets.sh on $stamp (placeholder values only — run ./secrets.sh for real credentials)"
  echo "# Region: $REGION"
  echo
} > "$tmp_example"
...
log "Wrote fresh .env and .env.example (placeholders)."
