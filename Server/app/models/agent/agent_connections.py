...
class AgentConnections(db.Model):
    ...
    last_read_at = db.Column(db.Text, nullable=True)  # JSON: {"agent_id": "2024-01-01T00:00:00", "client_id": "2024-01-01T00:00:00"}
    ...
    def update_last_read(self, user_id: str):
        ...
        if not self.last_read_at:
            ...
            try:
                last_read_dict = json.loads(self.last_read_at) if isinstance(self.last_read_at, str) else self.last_read_at
            except:
                last_read_dict = {}
    ...
    def get_last_read(self, user_id: str) -> Optional[datetime]:
    ...
    def __init__(self, **kwargs):
        super(AgentConnections, self).__init__(**kwargs)
        if not self.id:
