...
class AgreementParticipant(db.Model):
    ...
    user_id = db.Column(db.String(36), db.ForeignKey('users.id'), nullable=True)  # Nullable for external parties
    ...
    # Relationships
    agreement = db.relationship('Agreement', back_populates='participants', foreign_keys=[agreement_id])
    user = db.relationship('User', foreign_keys=[user_id])
    
    def __init__(self, **kwargs):
        super(AgreementParticipant, self).__init__(**kwargs)
        if not self.id:
