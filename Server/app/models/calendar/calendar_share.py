...
class CalendarShare(db.Model):
    ...
    # Relationships
    calendar_owner = db.relationship('User', foreign_keys=[calendar_owner_id], backref=db.backref('shared_calendars', lazy=True))
    shared_with_user = db.relationship('User', foreign_keys=[shared_with_user_id], backref=db.backref('calendars_shared_with_me', lazy=True))
    
    # Unique constraint: one calendar can only be shared with a user once
    __table_args__ = (db.UniqueConstraint('calendar_owner_id', 'shared_with_user_id', 'calendar_id', name='unique_calendar_share'),)
    
    def __init__(self, **kwargs):
        super(CalendarShare, self).__init__(**kwargs)
        if not self.id:
    ...
    def __repr__(self):
        return f'<CalendarShare {self.calendar_owner_id} -> {self.shared_with_user_id} ({self.role})>'
