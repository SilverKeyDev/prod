...
class AgentConnectionRequest(db.Model):
    ...
    requested_by_agent = db.Column(db.Boolean, nullable=False, default=False)  # True if agent requested, False if client requested
    ...
    # Relationships
    agent = db.relationship('User', foreign_keys=[agent_id], backref=db.backref('sent_agent_requests', lazy=True))
    client = db.relationship('User', foreign_keys=[client_id], backref=db.backref('received_agent_requests', lazy=True))
    
    def __init__(self, **kwargs):
        super(AgentConnectionRequest, self).__init__(**kwargs)
        if not self.id:
