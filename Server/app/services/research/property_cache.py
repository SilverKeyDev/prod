...
def find_cached_property(
    ...
    if zpid:
        candidate = HomeUniversal.query.filter_by(
            user_id=str(user_id),
            zpid=str(zpid)
        ).first()
...
def is_fully_populated(record: HomeUniversal) -> bool:
    ...
    essential_fields = [
        ...
        record.zpid or record.listing_status or record.property_type or record.home_type,  # Some identifier
    ]
...
def get_cached_details(
    ...
    details_commute_present = bool(
        record.commute_data and
        isinstance(record.commute_data, dict) and
        record.commute_data
    )
    details_analysis_present = bool(
        record.property_analysis and
        isinstance(record.property_analysis, dict) and
        record.property_analysis
    )
    details_features_present = bool(
        record.features and
        isinstance(record.features, dict)
    )
    ...
    force_regen = (not (details_commute_present and details_analysis_present and details_features_present)) and (not unlocked_recently)
