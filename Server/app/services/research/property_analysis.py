...
def prepare_user_preferences_dict(user_preferences: UserPreferences) -> Dict[str, Any]:
    """
    Convert UserPreferences model to dict format with JSON fields parsed.
    
    Args:
        user_preferences: UserPreferences model instance
        
    Returns:
        Dict with user preferences data
    """
...
def generate_property_analysis(
    ...
    try:
        ...
        if section_names and isinstance(section_names, list):
            ...
            if section_names:  # Only generate if there are sections to generate
                ...
                try:
                    ...
                    if current_user:
                        from app.services.research.property_research_pipeline import get_recent_property_analysis_sections
                    ...
                    current_app.logger.debug(f"[PROPERTY] Error checking recent sections: {recent_check_err}")
        ...
        if not skip_pros_cons and 'neighborhood_overview' not in property_analysis:
            current_app.logger.warning("⚠️ [PROPERTY] neighborhood_overview missing from response to frontend")
...
def get_property_analysis_for_property(
    ...
    if cached_property_analysis:
        current_app.logger.info("[PROPERTY] ⏭️ Skipping property_analysis generation, using cached data")
        ...
        if skip_pros_cons:
            ...
            current_app.logger.info("[PROPERTY] Removed pros/cons and neighborhood_overview from cached data")
    ...
    try:
        ...
        return generate_property_analysis(property_address, data, user_preferences, skip_pros_cons=skip_pros_cons)
