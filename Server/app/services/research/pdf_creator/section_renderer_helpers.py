...
def try_render_chart(key, v, elements, styles):
    ...
    if hasattr(v, "with_percent") and callable(v.with_percent):
        chart_data = v.with_percent()
        data_as_dict: dict[str, Any] = chart_data if isinstance(chart_data, dict) else {}
        if key.lower() == "lifestyle_dna":
            chart_buffer = generate_horizontal_bar_chart(data_as_dict, key)
            chart_type = "Lifestyle DNA Bar Chart"
        elif key.lower() == "age_distribution":
            chart_buffer = generate_vertical_lollipop_chart(data_as_dict, key)
        ...
        if key.lower() == "lifestyle_dna":
            ...
            chart_type = "Age Distribution Chart"
        elif key.lower() in CHARTABLE_FIELDS:
            chartable_values = 0
            total_values = len(v)
            for val in v.values():
                if val is None:
                ...
                if isinstance(val, str):
                    ...
                    if (
                        "/" in val_stripped
                        and val_stripped.split("/")[0].replace(".", "").isdigit()
                    ):
                        chartable_values += 1
                        continue
                    ...
                    if any(
                        c in val_stripped
                        for c in ["$", "-", "/", " to ", " and ", "month", "year", "per"]
                    ):
                        continue
