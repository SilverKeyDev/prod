"""Matplotlib chart generation for PNG output (shared across services)."""
...
def _resolve_colormap(primary: str, fallback: str = "viridis"):
    try:
        return colormaps[primary]
    except KeyError:
...
def generate_horizontal_bar_chart(data: dict, title: str) -> BytesIO | None:
    try:
        ...
        ax = cast(Axes, ax)
        cmap = _resolve_colormap("PuBuGn_r", "viridis")
