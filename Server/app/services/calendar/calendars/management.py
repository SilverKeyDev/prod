...
def delete_calendar(
    ...
    try:
        ...
        if e.resp.status == 404:
            ...
            logger.debug(f"Calendar {calendar_id} not found for user {user_id} (may already be deleted)")
            return True
        ...
        if e.resp.status == 403:
            ...
            if reason == "rateLimitExceeded" and retry_count < MAX_RETRIES:
                ...
                return delete_calendar(
                    user_id, calendar_id, client_id, client_secret, token_endpoint, scopes, retry_count + 1
                )
        ...
        logger.error(f"Error deleting calendar {calendar_id} for user {user_id}: {error_msg}", exc_info=True)
        raise
...
def get_or_create_silverkey_calendar(
    ...
    try:
            ...
            if len(silverkey_calendars) == 0:
                ...
                for index, cal in enumerate(calendars_to_delete):
                    ...
                    try:
                        ...
                        if index > 0:
                            logger.debug(f"Waiting {DELETE_DELAY_SECONDS} seconds before next calendar deletion to avoid rate limits")
                            time.sleep(DELETE_DELAY_SECONDS)
                        
                        delete_calendar(user_id, cal_id, client_id, client_secret, token_endpoint, scopes)
                        log_oauth_event("silverkey_calendar_deleted_duplicate", user_id, calendar_id=cal_id)
                        logger.info(f"Deleted duplicate SilverKey calendar {cal_id} for user {user_id} ({index + 1}/{len(calendars_to_delete)})")
        ...
        try:
            ...
            logger.info(f"Created SilverKey calendar {created_calendar.get('id')} for user {user_id}")
            return created_calendar
        except HttpError as e:
            ...
            if e.resp.status == 403:
                ...
                if reason == "quotaExceeded" and domain == "usageLimits":
                    log_oauth_event("silverkey_calendar_create_quota_exceeded", user_id, error=error_msg)
                    logger.error(
                ...
                if reason == "insufficientPermissions" or "insufficient authentication scopes" in error_details.get("message", "").lower():
