    ...
    try:
        ...
        if hasattr(e, 'resp') and e.resp.status == 403:
            ...
            is_insufficient_permissions = (
                ...
                "insufficient authentication scopes" in error_msg or
                "insufficient permissions" in error_msg
            )
            ...
            if is_insufficient_permissions and get_or_create_silverkey_calendar_func:
                logger.debug(f"User {user_id} has restricted scope (cannot list calendars), returning SilverKey calendar only")
                log_oauth_event("calendars_list_restricted_scope", user_id, reason="insufficientPermissions")
                ...
                try:
                    ...
                    logger.warning(f"Failed to get/create SilverKey calendar for user {user_id} with restricted scope: {error_msg}")
