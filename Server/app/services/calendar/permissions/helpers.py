...
def get_scopes_from_tokeninfo(access_token: str) -> Optional[str]:
...
def parse_scopes_to_permissions(scopes: str) -> Dict[str, bool]:
    ...
    """
    if not scopes:
        return {perm: False for perm in permissions.keys()}
...
def update_token_permissions_from_scopes(token_record, scopes: str) -> None:
    ...
    # Log raw input
    logger.info(f"PERMISSIONS_UPDATE_START", extra={
        'user_id': user_id,
    ...
    if not scopes:
        ...
        logger.info(f"PERMISSIONS_UPDATE_NO_SCOPES", extra={
            'user_id': user_id,
            'action': 'reset_all_permissions_to_false'
        })
    ...
    scope_list = [s.strip() for s in scopes.split() if s.strip()] if isinstance(scopes, str) else [str(s).strip() for s in scopes if s]
    scope_set = set(scope_list)  # Use set for exact matching
    
    # Log parsed scopes
    logger.info(f"PERMISSIONS_UPDATE_PARSED_SCOPES", extra={
        'user_id': user_id,
    ...
    def has_scope_or_implied(scope_url: str, implied_by: list = None) -> bool:
    ...
    logger.info(f"PERMISSIONS_UPDATE_COMPLETE", extra={
        'user_id': user_id,
        'raw_scopes_input': scopes,
...
def require_permission(user_id: str, permission: str, context: str = "this operation") -> Tuple[bool, Optional[Dict]]:
    ...
    scope_url = perm_data.get('scope_url', '')
...
def check_multiple_permissions(user_id: str, required_permissions: List[str], 
                               context: str = "this operation") -> Tuple[bool, Optional[Dict]]:
    ...
    reconnect_url = _build_reconnect_url(missing[0]) if missing else "/api/v1/google-calendar/oauth/start"
