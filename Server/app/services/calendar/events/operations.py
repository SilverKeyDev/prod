    ...
    try:
        ...
        if isinstance(e, HttpError) and hasattr(e, 'resp') and e.resp.status in [404, 403]:
            ...
            if e.resp.status == 403:
                ...
                from .error_handlers import extract_http_error_details
                ...
                if reason == "quotaExceeded" and domain == "usageLimits":
                    log_oauth_event("events_list_error", user_id, calendar_id=calendar_id_for_log, error="quota_exceeded")
                    logger.warning(
                        f"Calendar quota exceeded for user {user_id}, calendar {calendar_id_for_log} "
                        f"(requested: {calendar_id}): {error_msg}"
                    )
                    raise RuntimeError(
                        f"Google Calendar usage limit exceeded. "
                        f"Please wait before making more requests or reduce calendar activity."
                    )
                ...
                if reason == "insufficientPermissions" or "insufficient authentication scopes" in error_message.lower():
                    log_oauth_event("events_list_error", user_id, calendar_id=calendar_id_for_log, error="authentication_failed")
                    logger.warning(
                        f"Calendar authentication failed for user {user_id}, calendar {calendar_id_for_log} "
                        f"(requested: {calendar_id}): {error_msg}"
                    )
                    raise RuntimeError(
                        f"Google Calendar authentication failed. "
                        f"Please reconnect your Google Calendar account with appropriate permissions."
                    )
