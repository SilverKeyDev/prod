...
SCOPE_BY_URL: Dict[str, GoogleScope] = {
    scope.url: scope for scope in SCOPE_BY_NAME.values()
}
...
def get_scope_by_url(scope_url: str) -> Optional[GoogleScope]:
...
def get_scope_by_name(scope_name: str) -> Optional[GoogleScope]:
...
def normalize_scope_url(scope_url: str) -> Optional[str]:
    ...
    for url, scope in SCOPE_BY_URL.items():
