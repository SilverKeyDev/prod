"""Search inactive/sold listings via Slipstream /ws/listings/inactive/search.
...
def search_inactive_listings(
    polygon_geojson: dict | None = None,
    filters: dict[str, Any] | None = None,
    page_number: int = 1,
    page_size: int = 200,
) -> tuple[list[dict], dict, list[dict]]:
    """Search inactive/sold listings with optional polygon and filters.
    ...
    try:
        ...
        log.debug(
            "API",
            "Slipstream /ws/listings/inactive/search response (inactive, first home only)",
            {
                "caller": "search_inactive_listings",
