...
class FeatureEngineer:
    ...
    def calculate_price_features(
        self, user_prefs: dict[str, Any], home_data: dict[str, Any]
    ) -> list[float]:
    ...
    def calculate_location_features(
        ...
        home_neighborhood = str(home_data.get("neighborhood", "")).lower()

        if preferred_neighborhoods and home_neighborhood:
            neighborhood_match = (
                1.0
                if any(str(pref).lower() in home_neighborhood for pref in preferred_neighborhoods)
                else 0.0
            )
        else:
    ...
    def calculate_categorical_features(
        ...
        # Home type match
        preferred_types = user_prefs.get(
            "preferred_home_types", ["single_family"]
        )  # Default preference
        home_type = str(home_data.get("home_type", "single_family")).lower()  # Default type

        if preferred_types and home_type:
            type_match = (
                1.0 if any(str(pref).lower() == home_type for pref in preferred_types) else 0.0
            )
        else:
        ...
        home_style = str(home_data.get("style", "traditional")).lower()  # Default style

        if preferred_styles and home_style:
            style_match = (
                1.0 if any(str(pref).lower() == home_style for pref in preferred_styles) else 0.0
            )
        else:
    ...
    def calculate_amenity_features(
        self, user_prefs: dict[str, Any], home_data: dict[str, Any]
    ) -> list[float]:
        ...
        _home_amenities = home_data.get("amenities", ["garage", "kitchen"])
        home_amenities: list[str] = (
            _home_amenities if isinstance(_home_amenities, list) else ["garage", "kitchen"]
        )

        if must_have_amenities:
            home_amenities_lower = [str(a).lower() for a in home_amenities]
            must_have_count = sum(
                1
                for amenity in must_have_amenities
                if any(str(amenity).lower() in ha for ha in home_amenities_lower)
            )
        ...
        if nice_to_have_amenities:
            ...
            nice_to_have_count = sum(
                1
                for amenity in nice_to_have_amenities
                if any(str(amenity).lower() in ha for ha in home_amenities_lower)
            )
    ...
    def get_feature_names(self) -> list[str]:
        ...
        return [
            ...
            "commute_score",
            "commute_with_tolerance",
            "neighborhood_match",
            # Categorical features (2)
            "home_type_match",
            "style_match",
            # Amenity features (2)
            "must_have_amenity_ratio",
            "nice_to_have_amenity_ratio",
        ]
