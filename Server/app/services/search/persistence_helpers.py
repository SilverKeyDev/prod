    ...
    try:
        ...
        for index, prop in enumerate(properties, start=1):
            try:
                ...
                home_record = add_or_update_home_basic(user_id=str(user_id), home=prop, set_liked=False, ranking=ranking)
        ...
        marked_count = 0
        unliked = HomeUniversal.query.filter_by(user_id=str(user_id), is_liked=False, current=True).all()
        for h in unliked:
            keep = False
        ...
        if marked_count:
            db.session.commit()
            current_app.logger.info(f"🏷️ Marked {marked_count} unliked homes as not current for user {user_id}")
