...
def get_agent_clients(agent_id: str) -> list[dict]:
    ...
    try:
        ...
        if not agent:
            logger.warning("Agent %s not found or not an agent", agent_id)
            return []
        ...
        logger.error("Error fetching clients for agent %s: %s", agent_id, e, exc_info=True)
        raise
...
def get_client_info(client_id: str) -> dict | None:
    ...
    try:
        ...
        logger.error("Error fetching client info for %s: %s", client_id, e, exc_info=True)
        raise
...
def agent_may_access_client(agent_id: str, client_id: str) -> bool:
    ...
    try:
        ...
        logger.error("Error validating agent-client relationship: %s", e, exc_info=True)
        return False
...
def get_user_agent_id(user_id: str) -> str | None:
    ...
    try:
        ...
        logger.error("Error getting agent ID for user %s: %s", user_id, e, exc_info=True)
        return None
...
def get_connected_agent_ids_for_client(client_id: str) -> set[str]:
    ...
    try:
        ...
        logger.error(
            "Error resolving connected agents for client %s: %s", client_id, e, exc_info=True
        )
        raise
...
def get_agent_client_ids(agent_id: str) -> list[str]:
    ...
    try:
        ...
        logger.error("Error getting client IDs for agent %s: %s", agent_id, e, exc_info=True)
        return []
