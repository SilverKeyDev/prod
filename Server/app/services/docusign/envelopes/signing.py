...
from app.config import Config, DOCUSIGN_SIGNING_COMPLETE_PATH, DOCUSIGN_SENDER_VIEW_PATH
...
from ..errors import AgreementStateError, ParticipantNotFoundError
...
class SigningService:
    ...
    def get_signing_url(agreement: Agreement, participant: AgreementParticipant) -> str:
        ...
        Returns:
            Signing URL
        """
        logger.debug(LOG_CATEGORIES["DOCUSIGN"], "Getting signing URL for participant", {
        ...
        if not agreement.docusign_envelope_id:
            logger.warn(LOG_CATEGORIES["DOCUSIGN"], "Cannot get signing URL - no envelope ID", {
                "agreement_id": agreement.id
            })
            raise AgreementStateError("Agreement not sent to DocuSign")
        
        if agreement.status not in ['sent', 'delivered']:
            logger.warn(LOG_CATEGORIES["DOCUSIGN"], "Cannot get signing URL - invalid status", {
                "agreement_id": agreement.id,
        ...
        logger.debug(LOG_CATEGORIES["DOCUSIGN"], "Building recipient data for signing", {
            "agreement_id": agreement.id,
            "participant_id": participant.id,
        ...
        logger.debug(LOG_CATEGORIES["DOCUSIGN"], "Calling DocuSign to create recipient view", {
            "agreement_id": agreement.id,
    ...
    def get_sender_view_url(agreement: Agreement) -> str:
        ...
        Returns:
            Sender view URL
        """
        logger.debug(LOG_CATEGORIES["DOCUSIGN"], "Getting sender view URL", {
            "agreement_id": agreement.id,
            "envelope_id": agreement.docusign_envelope_id
        })
        
        if not agreement.docusign_envelope_id:
            logger.warn(LOG_CATEGORIES["DOCUSIGN"], "Cannot get sender view URL - no envelope ID", {
                "agreement_id": agreement.id
            })
            raise AgreementStateError("Agreement not sent to DocuSign")
        
        # Get return URL
        return_url = f"{Config.FRONTEND_URL}{DOCUSIGN_SENDER_VIEW_PATH.format(agreement_id=agreement.id)}"
        
        logger.debug(LOG_CATEGORIES["DOCUSIGN"], "Calling DocuSign to create sender view", {
            "agreement_id": agreement.id,
            "envelope_id": agreement.docusign_envelope_id,
            "return_url": return_url
        })
        ...
        logger.info(LOG_CATEGORIES["DOCUSIGN"], "Sender view URL generated successfully", {
            "agreement_id": agreement.id,
            "envelope_id": agreement.docusign_envelope_id
        })
