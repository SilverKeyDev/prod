...
def can_void_agreement(user: User, agreement: Agreement) -> bool:
    ...
    return (user.id == agreement.agent_id and 
            agreement.status not in ['completed', 'voided'])


def can_get_signing_url(user: User, agreement: Agreement, participant: AgreementParticipant) -> bool:
    """
...
def is_agent(user: Optional[User]) -> bool:
    """
...
def can_access_oauth(user: Optional[User]) -> bool:
    """
