...
def handle_login(data: dict[str, Any], request_id: str) -> tuple[Response, int]:
    ...
    result = AWS_COGNITO_service.sign_in(username=data["email"], password=data["password"])
    try:
        success_value = result["success"]
    except KeyError as key_error:
        log.error(
            "ERRORS",
            "AUTH_LOGIN_SUCCESS_KEY_ERROR",
            {
    ...
    if not success_value:
        if result.get("needs_verification") or result.get("error") == "UserNotConfirmedException":
            try:
                ...
                log.info(
                    "AUTH",
                    "AUTH_LOGIN_UNVERIFIED_CODE_SENT",
                    {"request_id": request_id, "email": mask_email(data.get("email", ""))},
                )
                return (
                    make_response(
                        {
                ...
                log.error(
                    "ERRORS",
                    "AUTH_LOGIN_RESEND_CODE_ERROR",
                    {"request_id": request_id, "error": str(resend_error)},
                )
                return (
                    make_response(
                        {
        ...
        log.warn(
            "AUTH", "AUTH_LOGIN_FAILED", {"request_id": request_id, "duration_ms": duration_ms}
        )
        return (
            make_response(
                {
    ...
    if user:
        set_person_properties(user_id, properties={"has_agent_role": user_is_agent(user)})
        capture_product_event(user_id, "user_logged_in", properties={"login_method": "password"})
    return (resp, 200)
