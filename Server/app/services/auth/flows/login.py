...
def handle_login(data: dict[str, Any], request_id: str) -> tuple[Response, int]:
    ...
    if not success_value:
        ...
        if result.get("needs_verification") or result.get("error") == "UserNotConfirmedException":
            try:
                ...
                current_app.logger.info(
                    ...
                    extra={"request_id": request_id, "email": mask_email(data.get("email", ""))},
                )
                return make_response(
                    {
                        "success": False,
                ...
                current_app.logger.error(
                    ...
                    extra={"request_id": request_id, "error": str(resend_error)},
                )
                return make_response(
                    {
                        "success": False,
        ...
        current_app.logger.warning(
            "AUTH_LOGIN_FAILED", extra={"request_id": request_id, "duration_ms": duration_ms}
        )
