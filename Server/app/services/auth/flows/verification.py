...
def handle_verify_email(data: dict[str, Any], request_id: str) -> tuple[dict[str, Any], int]:
    ...
    return ({"success": False, "message": result.get("message", "Failed to verify user")}, 400)
...
def handle_verification(data: dict[str, Any], request_id: str) -> tuple[Response, int]:
    ...
    try:
        ...
        if user:
            capture_product_event(user_id, "user_email_verified", properties={"auto_login": True})
        return (resp, 200)
...
def handle_resend_code(
    ...
    try:
        ...
        log.error("ERRORS", f"Error resending verification code: {str(e)}")
        return (
            {
                "success": False,
