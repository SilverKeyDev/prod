...
def _decode_session_cookie(
    ...
    try:
        ...
        log.error(
            ...
            {"request_id": request_id, "error": str(decode_error), "duration_ms": duration_ms},
        )
        return (None, None)
...
def _resolve_user_from_refresh_token_only(refresh_token: str) -> User | None:
    ...
    google_token = db.session.scalar(
        select(GoogleOAuthToken).where(GoogleOAuthToken.refresh_token == refresh_token)
    )
    if google_token:
        return db.session.scalar(select(User).where(User.id == google_token.user_id))
    return None
