...
def ensure_cognito_account_for_user(email: str) -> Tuple[Optional[str], Optional[str], bool]:
    """
    ...
    try:
        ...
        if not user:
            ...
            current_app.logger.warning(f"ENSURE_COGNITO_USER_NOT_FOUND", extra={
        ...
        if user.cognito_id:
            masked_email = mask_email(email)
            current_app.logger.debug(f"ENSURE_COGNITO_ALREADY_EXISTS", extra={
                'request_id': request_id,
                'email': masked_email,
        ...
        if not user.google_id:
            masked_email = mask_email(email)
            current_app.logger.warning(f"ENSURE_COGNITO_NOT_GOOGLE_USER", extra={
                'request_id': request_id,
                'email': masked_email,
                'note': 'User exists but has neither cognito_id nor google_id'
            })
            return None, f"User account exists but is not a Google OAuth user. Please contact support.", False
        ...
        masked_email = mask_email(email)
        current_app.logger.info(f"ENSURE_COGNITO_CREATING_ACCOUNT", extra={
            'request_id': request_id,
            'email': masked_email,
            'user_id': user.id
        })
        ...
        if not result['success']:
            ...
            current_app.logger.error(f"ENSURE_COGNITO_CREATION_FAILED", extra={
                'request_id': request_id,
                'email': masked_email,
        ...
        try:
            ...
            current_app.logger.info(f"ENSURE_COGNITO_ACCOUNT_CREATED", extra={
                'request_id': request_id,
                'email': masked_email,
                'user_id': user.id,
            ...
            if not set_password_result['success']:
                current_app.logger.error(f"ENSURE_COGNITO_SET_PASSWORD_FAILED", extra={
                    'request_id': request_id,
                    'email': masked_email,
            ...
            try:
                ...
                if status_after_set['success']:
                    current_app.logger.info(f"ENSURE_COGNITO_STATUS_AFTER_SET_PASSWORD", extra={
                        'request_id': request_id,
                        'email': masked_email,
                    ...
                    if status_after_set.get('user_status') != 'CONFIRMED':
                        current_app.logger.warning(f"ENSURE_COGNITO_STATUS_NOT_CONFIRMED", extra={
                            'request_id': request_id,
                            'email': masked_email,
                            'user_status': status_after_set.get('user_status'),
                            'expected': 'CONFIRMED',
                ...
                current_app.logger.debug(f"ENSURE_COGNITO_STATUS_CHECK_FAILED", extra={
                    'request_id': request_id,
                    'email': masked_email,
            ...
            current_app.logger.info(f"ENSURE_COGNITO_CONVERTED_TO_CONFIRMED", extra={
                'request_id': request_id,
                'email': masked_email,
            ...
            db.session.rollback()
            current_app.logger.error(f"ENSURE_COGNITO_DB_UPDATE_FAILED", extra={
                'request_id': request_id,
                'email': masked_email,
                'error': str(db_error),
                'cognito_id': cognito_id[:8] + '...' if cognito_id else None
        ...
        current_app.logger.error(f"ENSURE_COGNITO_UNEXPECTED_ERROR", extra={
            'request_id': request_id,
            'email': masked_email,
            'error': str(e),
