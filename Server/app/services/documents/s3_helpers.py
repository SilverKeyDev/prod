...
def parse_s3_key_path(s3_key: str) -> dict[str, str] | None:
    """
    ...
    if len(parts) >= 4 and parts[1] == "reports":
        return {"user_id": parts[0], "report_type": parts[2], "filename": "/".join(parts[3:])}

    return None


def get_json_key_from_pdf_key(pdf_key: str) -> str | None:
    """
