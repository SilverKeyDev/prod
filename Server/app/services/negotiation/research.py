    ...
    *,
    ...
    for attempt in range(max_retries + 1):
        ...
        try:
            ...
            log.error(
                "API",
                "Negotiation Perplexity request failed",
        ...
        try:
            ...
            log.error(
                "ERRORS",
                "Negotiation Perplexity response decode failed",
        ...
        if "choices" not in content or not content["choices"]:
            ...
            log.error(
                "ERRORS",
                "Negotiation Perplexity malformed response",
                {"section": section_name, "error": last_error},
            )
            if attempt < max_retries:
                continue
        ...
        try:
            ...
            log.error(
                "ERRORS",
                "Negotiation strategy parse failed",
                {"section": section_name, "error": last_error},
            )
            if attempt < max_retries:
                continue
