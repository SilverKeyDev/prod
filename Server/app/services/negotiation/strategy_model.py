...
class Comp(BaseModel):
    ...
    living_sqft: int | None = Field(default=None, ge=0)
    lot_sqft: int | None = Field(default=None, ge=0)
    condition_notes: str | None = None
    adjustments_notes: str | None = None
...
class PriceSection(BaseModel):
    ...
    credits_and_terms: list[str] = Field(
        default_factory=list, description="Requested seller credits and terms"
    )
...
class NegotiationStrategy(BaseModel):
    ...
    def from_user_preferences(
        ...
        **overrides,
        ...
        def create_detailed_price_rationale(
            target_property: dict | None, comps: list[dict], opening_amount: Decimal
        ) -> str:
            ...
            if target_property:
                ...
                target_property.get("address", "this property")
            ...
            for i, comp in enumerate(comps[:3]):  # Limit to top 3 comps
                ...
                comp.get("sold_date", comp.get("date"))
            ...
            if price_range:
                ...
                if target_price:
                    if opening_amount < min_price:
                        ...
                        positioning = "competitive positioning within comp range"
                else:
                    positioning = "strategic positioning at 95% of budget"

                rationale = (
                    f"Opening at ${opening_amount:,.0f} based on detailed comparable analysis:\n\n"
                )
                rationale += "COMPARABLE SALES ANALYSIS:\n"
                rationale += "\n".join(comp_analysis)
                rationale += f"\n\nPRICE POSITIONING: {positioning}\n"
                rationale += (
                    f"Comp range: ${min_price:,.0f} - ${max_price:,.0f} (avg: ${avg_price:,.0f})\n"
                )
        ...
        if urgency_level == "ready_to_buy":
            timeline = "Close within 21–30 days; flexible on seller timeline"
        elif urgency_level == "just_looking":
            timeline = "Flexible closing timeline; no rush"
        else:
            timeline = "Standard 30–45 day closing timeline"
        ...
        if down_payment > float(home_budget_max) * 0.2:
            offer_strength = (
                "Strong down payment (>20%) with pre-approval eliminates financing risk"
            )
        else:
        ...
        # Apply any overrides
        data = {"price_section": price_section, "market_section": market_section, **overrides}
