"""
JSON cleanup and validation utilities for negotiation report generation.
"""
...
def validate_address(address: str) -> bool:
    if not address or not isinstance(address, str):
        log.error("NEGOTIATION", "Address is empty or not a string")
        return False
    if len(address.strip()) == 0:
        log.error("NEGOTIATION", "Address is empty after stripping whitespace")
        return False
    return True
...
def _safe_parse_json(text: str, report_customization: dict | None = None) -> dict:
    try:
        ...
        if not isinstance(parsed, dict):
            raise ValueError("Structured JSON root must be an object (dict)")
        return parsed
