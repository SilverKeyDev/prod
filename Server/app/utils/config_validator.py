...
def _load_env_example_keys() -> Set[str]:
    ...
    if not ENV_EXAMPLE_PATH.exists():
        logger.warning(f".env.example file not found at {ENV_EXAMPLE_PATH}. Using fallback required variables.")
        return env_keys
    ...
    try:
        ...
        if env_keys:
            logger.info(f"Loaded {len(env_keys)} required environment variables from {ENV_EXAMPLE_PATH}")
        else:
...
def validate_environment() -> Tuple[bool, List[str]]:
    ...
    logger.info(f"All {len(REQUIRED_ENV_VARS)} required environment variables are set (validated against .env.example)")
    return True, []
