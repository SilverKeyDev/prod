...
class FlexibleEnvValidator:
    ...
    CATEGORY_PATTERNS = {
        ...
        'External APIs': [r'.*API[_-]?KEY.*', r'.*RAPIDAPI.*', r'.*GOOGLE.*', r'.*PERPLEXITY.*', r'.*CENSUS.*'],
    ...
    def _is_system_variable(cls, name: str) -> bool:
        ...
        system_prefixes = [
            ...
            'XDG_', 'DISPLAY', 'SSH_', 'TMPDIR', 'PWD', 'OLDPWD',
            'CONDA_', 'VIRTUAL_ENV', 'PYTHONPATH', 'PIP_', 'npm_',
            'NODE_', 'VSCODE_', 'EDITOR', 'PAGER'
        ]
    ...
    @classmethod
    def get_env_var(cls, name: str, default: Optional[str] = None, required: bool = False) -> Optional[str]:
        """
            ...
            if var_names:
                ...
                for var_name in sorted(var_names):
                    ...
                    if env_info.is_critical:
                        template_lines.append(f"# ⚠️ CRITICAL: {var_name} is required for application to function")
