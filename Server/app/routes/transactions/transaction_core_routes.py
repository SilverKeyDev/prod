...
from .handlers._helpers import me_transaction_payload
...
def list_transactions(user):
    ...
    try:
        rows = list_transactions_for_actor(
            str(user.id),
            buyer_id=buyer_id,
...
def create_transaction_route(user, data: CreateTransactionRequest):
    ...
    try:
        ...
        if is_agent and not tx.primary_agent_id:
            tx.primary_agent_id = str(user.id)
        db.session.commit()
...
def get_my_transaction(user):
    ...
    tx = resolve_active_transaction(buyer_id=str(user.id))
    db.session.commit()
