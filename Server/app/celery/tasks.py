...
def train_user_weights_task(self, user_id: str, force: bool = False):
    ...
    try:
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Training user weights', 'progress': 10}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Extracting training data', 'progress': 30}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Training complete', 'progress': 100}
        )
        ...
        current_app.logger.error(f"[WEIGHT_TRAINING] Task error for user {user_id}: {e}", exc_info=True)
        return {
            'success': False,
            'error': str(e),
            'user_id': user_id
...
def train_all_eligible_users_task(self, limit: int = 100):
    ...
    try:
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Starting batch weight training', 'progress': 5}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Processing users', 'progress': 30}
        )
        ...
        self.update_state(
            state='PROGRESS',
            meta={'status': 'Batch training complete', 'progress': 100}
        )
