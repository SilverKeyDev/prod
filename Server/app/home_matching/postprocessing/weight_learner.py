...
class WeightLearner:
    ...
    def fit(
        ...
        try:
            ...
            X = np.array([
                [ex['embedding_score'], ex['llm_score']]
            ...
            # Convert coefficients to weights (normalize to sum to 1, use absolute values)
            # Since coefficients can be negative, we use absolute values and normalize
            ...
            logger.info(f"Trained model: embedding_weight={embedding_weight:.3f}, "
                       f"llm_weight={llm_weight:.3f}, accuracy={accuracy:.3f}, auc={auc:.3f}")
            
            return embedding_weight, llm_weight, accuracy, auc
    ...
    def update_cohort_weights(
        ...
        try:
            ...
            logger.info(f"Updated weights for cohort {cohort_id}: "
                       f"embedding={embedding_weight:.3f}, llm={llm_weight:.3f}")
