...
from app import db
from app.models import User, UserScoreWeights
...
class WeightTrainingJob:
    ...
    def train_user_weights(self, user_id: str, force: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            training_examples, num_positive, num_negative = training_data_extractor.extract_user_training_data(user_id)
    ...
    def train_cohort_weights(self, cohort_id: str, user_ids: List[str]) -> Dict[str, Any]:
        ...
        try:
            ...
            training_examples, num_positive, num_negative = training_data_extractor.extract_cohort_training_data(
                cohort_id, user_ids
            )
    ...
    def train_all_eligible_users(self, limit: int = 100) -> Dict[str, Any]:
        ...
        try:
            # Get all users
            users = User.query.filter(User.has_preferences == True).limit(limit).all()
            ...
            logger.info(f"Trained weights for {results['trained']} users, "
    ...
    def initialize_cohort_weights(self, cohort_id: str = None) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if cohort_id == cohort_assigner.DEFAULT_COHORT:
                users = User.query.filter(User.has_preferences == False).all()
            else:
...
def train_user_weights_task(user_id: str, force: bool = False):
    ...
    Usage:
        from app.home_matching.postprocessing.weight_training_job import train_user_weights_task
...
def train_all_eligible_users_task(limit: int = 100):
    ...
    Usage:
        from app.home_matching.postprocessing.weight_training_job import train_all_eligible_users_task
