...
from app import db
from app.models import HomeUniversal, HomeLikes, ScoringResultsTracker
...
class WeightTrainingDataExtractor:
    ...
    def extract_user_training_data(
        ...
        try:
            ...
            impressions = HomeUniversal.query.filter(
                HomeUniversal.user_id == user_id,
                HomeUniversal.created_at >= cutoff_date
            ).all()
            
            if len(impressions) < self.min_impressions:
                logger.debug(f"User {user_id} has only {len(impressions)} impressions, need {self.min_impressions}")
                return [], 0, 0
            
            # Get positive labels (liked homes)
            liked_homes = HomeLikes.query.filter(
                HomeLikes.user_id == user_id,
                HomeLikes.is_liked == True
            ).all()
            
            liked_home_ids = {str(home.id) for home in liked_homes}
            liked_addresses = {self._normalize_address(home.address) for home in liked_homes if home.address}
            ...
            for impression in impressions:
                # Determine label
                is_positive = (
                    str(impression.id) in liked_home_ids or
                    (impression.address and self._normalize_address(impression.address) in liked_addresses) or
                    impression.is_liked
                )
                
                # Get subscores from ScoringResultsTracker
                # Try to find recent scoring events for this home
                score_event = ScoringResultsTracker.query.filter(
                ...
                if not score_event and impression.address:
                    ...
                    all_events = ScoringResultsTracker.query.filter(
                        ScoringResultsTracker.user_id == user_id
                    ).order_by(ScoringResultsTracker.created_at.desc()).limit(1000).all()
            ...
            if num_positive < self.min_positive:
                logger.debug(f"User {user_id} has only {num_positive} positive examples, need {self.min_positive}")
                return [], 0, 0
            
            logger.info(f"Extracted {len(training_examples)} training examples for user {user_id} "
    ...
    def extract_cohort_training_data(
        ...
        try:
            ...
            logger.info(f"Extracted {len(all_examples)} training examples for cohort {cohort_id} "
                       f"({total_positive} positive, {total_negative} negative)")
            
            return all_examples, total_positive, total_negative
            
        except Exception as e:
            logger.error(f"Error extracting training data for cohort {cohort_id}: {e}", exc_info=True)
            return [], 0, 0
    ...
    def get_training_data_summary(self, user_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                'has_sufficient_data': len(examples) >= self.min_impressions and num_positive >= self.min_positive,
