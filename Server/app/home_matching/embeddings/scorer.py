...
class EmbeddingScorer:
    ...
    def score_user_against_homes(
        ...
        try:
            ...
            amplified_similarities = [
                self._amplify_embedding_score(sim) for sim in raw_similarities
            ]

            # Combine homes with their amplified scores
            scored_homes = list(zip(homes_data, amplified_similarities, strict=False))
    ...
    def score_multiple_users_against_homes(
        self, users_data: list[dict[str, Any]], homes_data: list[dict[str, Any]]
        ...
        try:
            ...
            for user_data, user_embedding in zip(users_data, user_embeddings, strict=False):
                similarities = self.similarity_calculator.calculate_multiple(
                    user_embedding, home_embeddings
                )

                # Combine homes with their scores
                scored_homes = list(zip(homes_data, similarities, strict=False))
                scored_homes.sort(key=lambda x: x[1], reverse=True)
    ...
    def get_top_matches(
        self, user_data: dict[str, Any], homes_data: list[dict[str, Any]], top_k: int = 10
    ) -> list[tuple[dict[str, Any], float]]:
    ...
    def explain_similarity(
        ...
        try:
            ...
            cosine_sim = self.similarity_calculator.calculate(
                user_embedding, home_embedding, "cosine"
            )
            dot_product_sim = self.similarity_calculator.calculate(
                user_embedding, home_embedding, "dot_product"
            )
            euclidean_sim = self.similarity_calculator.calculate(
                user_embedding, home_embedding, "euclidean"
            )
            ...
            explanation = {
                ...
                "home_features": {
                    ...
                    "sqft": f"{home_data.get('sqft', 0):,} sq ft"
