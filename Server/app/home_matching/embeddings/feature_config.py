...
class FeatureExtractor:
    ...
    def __init__(self, name: str, extractor_func: Callable, normalizer: float = 1.0, default_value: Any = 0):
        self.name = name
...
SHARED_FEATURES = {
    ...
    'price_budget': FeatureExtractor(
        name='price_budget',
        extractor_func=lambda data: data.get('price', data.get('budget_max', data.get('preferences', {}).get('budget_max', 0))),
        normalizer=1000000,
        default_value=0
    ),
    ...
    'bedrooms': FeatureExtractor(
        name='bedrooms',
        extractor_func=lambda data: data.get('bedrooms', data.get('preferred_bedrooms', data.get('preferences', {}).get('preferred_bedrooms', 0))),
        normalizer=10,
        default_value=0
    ),
    ...
    'bathrooms': FeatureExtractor(
        name='bathrooms',
        extractor_func=lambda data: data.get('bathrooms', data.get('preferred_bathrooms', data.get('preferences', {}).get('preferred_bathrooms', 0))),
        normalizer=10,
        default_value=0
    ),
    ...
    'sqft': FeatureExtractor(
        name='sqft',
        extractor_func=lambda data: data.get('sqft', data.get('min_sqft', data.get('preferences', {}).get('min_sqft', 0))),
        normalizer=10000,
        default_value=0
    ),
    ...
    'commute_time': FeatureExtractor(
        name='commute_time',
        extractor_func=lambda data: data.get('commute_minutes', data.get('max_commute_minutes', data.get('preferences', {}).get('max_commute_minutes', 30))),
    ...
    'pet_friendly': FeatureExtractor(
        name='pet_friendly',
        extractor_func=lambda data: 1.0 if data.get('pet_friendly', data.get('preferences', {}).get('pet_friendly', False)) else 0.0,
        normalizer=1.0,
        default_value=0.0
    ),
}
...
USER_SPECIFIC_FEATURES = {
    ...
    'budget_range': FeatureExtractor(
        name='budget_range',
        extractor_func=lambda data: max(0, data.get('preferences', {}).get('budget_max', 0) - data.get('preferences', {}).get('budget_min', 0)),
        normalizer=1000000,
        default_value=0
    ),
    ...
    'parking_required': FeatureExtractor(
        ...
        extractor_func=lambda data: 1.0 if data.get('preferences', {}).get('parking_required', False) else 0.0,
        normalizer=1.0,
        default_value=0.0
    ),
    ...
    'outdoor_space_required': FeatureExtractor(
        ...
        extractor_func=lambda data: 1.0 if data.get('preferences', {}).get('outdoor_space_required', False) else 0.0,
        normalizer=1.0,
        default_value=0.0
    ),
}
...
class FeatureConfig:
    ...
    @staticmethod
    def get_embedding_dimension(embedding_provider: str = "sentence_transformer", model: str = None) -> Dict[str, int]:
