...
def upgrade():
    ...
    op.create_table('agent_connection_requests',
        sa.Column('id', sa.String(length=36), nullable=False),
        sa.Column('agent_id', sa.String(length=36), nullable=False),
        sa.Column('client_id', sa.String(length=36), nullable=False),
    ...
    with op.batch_alter_table('chat_history', schema=None) as batch_op:
        ...
        batch_op.create_foreign_key('fk_chat_history_conversation', 'agent_conversations', ['conversation_id'], ['id'])
    ...
    op.create_index('ix_agent_connection_requests_agent_id', 'agent_connection_requests', ['agent_id'])
    op.create_index('ix_agent_connection_requests_client_id', 'agent_connection_requests', ['client_id'])
