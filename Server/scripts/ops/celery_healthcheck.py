#!/usr/bin/env python3
"""
Purpose:  Verify connectivity to the Celery broker URL (same as the Flask app).
Called by: Manual — ops/monitoring only. Not wired to any Makefile target or CI workflow.
...
Example (from repo root, with Server venv active):

    cd Server && python scripts/celery_healthcheck.py
"""
...
def main() -> int:
    ...
    env_path = os.path.join(_SERVER_ROOT, ".env")
