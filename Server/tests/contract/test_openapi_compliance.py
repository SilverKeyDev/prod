...
def test_all_routes_have_validation():
    ...
    if unvalidated_routes:
        print("\n⚠️  Routes without OpenAPI validation:")
        for route in sorted(unvalidated_routes):
            print(f"  - {route}")
        print(f"\nTotal: {len(unvalidated_routes)} routes need migration")
        
        # Uncomment after migration is complete:
        # pytest.fail(f"{len(unvalidated_routes)} routes missing validation")
    else:
        print("✅ All routes have OpenAPI validation")
...
def test_schemas_available():
    ...
    try:
        ...
        print("✅ Generated schemas are available")
