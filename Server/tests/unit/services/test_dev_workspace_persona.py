...
def _create_user(*, email: str, cognito_id: str, is_agent: bool = False) -> User:
    user = User(
...
class TestApplyDevWorkspacePersona:
    @pytest.mark.parametrize(
        ("workspace", "expected_is_agent", "expected_roles"),
        [
    ...
    def test_sets_exclusive_persona(
        self, app, db_session, workspace, expected_is_agent, expected_roles
    ) -> None:
        with app.app_context():
            ...
            assert user.is_agent is expected_is_agent
            rows = {r.role for r in UserRole.query.filter_by(user_id=user.id).all()}
    ...
    def test_syncs_buyer_demographics(self, app, db_session) -> None:
        with app.app_context():
            ...
            demo = UserDemographics.query.filter_by(user_id=user.id).one()
