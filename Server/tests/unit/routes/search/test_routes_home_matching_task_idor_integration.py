"""Integration-style IDOR checks for home-matching Celery task-status."""
...
class TestHomeMatchingTaskIdorIntegration:
    ...
    def test_task_status_denied_for_non_owner(self, client):
        register_task_owner("task-idor-hm", "user-owner-a")

        with patch(MOCK_JWT_USER) as mock_user_fn:
            mock_user_fn.return_value = _user("user-intruder-b")
            response = client.get(
                "/api/home-matching/task-status/task-idor-hm",
    ...
    def test_task_status_allowed_for_owner(self, client):
        register_task_owner("task-idor-hm-ok", "user-owner-a")
        ...
        with patch(MOCK_JWT_USER) as mock_user_fn:
            ...
            with patch("app.routes.search.home_matching.celery.AsyncResult") as mock_result:
                ...
                mock_task.result = {"matches": []}
                mock_result.return_value = mock_task
                response = client.get(
                    "/api/home-matching/task-status/task-idor-hm-ok",
                    headers={"Authorization": f"Bearer {MOCK_JWT_TOKEN}"},
                )
