...
class TestResearchPreferencesContext(unittest.TestCase):
    ...
    @patch(
        "app.services.search.scoring.research_preferences_context.user_is_agent",
        return_value=False,
    )
    def test_buyer_ignores_foreign_preferences_user_id(self, _mock_is_agent) -> None:
        user = MagicMock()
        user.id = "buyer-1"
    ...
    def test_agent_forbidden_when_client_not_in_list(self, _mock_is_agent, mock_may_access) -> None:
        user = MagicMock()
        user.id = "agent-1"
    ...
    def test_agent_allowed_for_listed_client(self, _mock_is_agent, mock_may_access) -> None:
        user = MagicMock()
        user.id = "agent-1"
    ...
    def test_agent_allowed_via_connection_only_mocked(
        self, _mock_is_agent, mock_may_access
    ) -> None:
        user = MagicMock()
        user.id = "agent-conn"
        mock_may_access.return_value = True
