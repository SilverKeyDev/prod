#!/usr/bin/env bash
# Purpose:  Fail when macOS/iCloud/Finder conflict copies exist (e.g. "file 2.py" beside "file.py").
# Called by: scripts/ci/run-all-linters.sh (first step before any lint gate)
set -euo pipefail
